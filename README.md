...
- `tools/secparams-codegen`: regenerates security parameter tables (`pkg/base/constants.go`).
- `tools/field-codegen`: regenerates finite field arithmetic (via Fiat Crypto) used by several curves).
...
This builds a container from `build.Dockerfile`, mounts the repo at `/src`, and sets `BUILD_PREFIX=docker-`
