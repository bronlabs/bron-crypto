...
This library is not designed to be fully side-channel resistant. We build higher-level protocols on top of a side-channel resistant foundation of elliptic curve (usually generated by fiat-cryptography, for example see [K256](./pkg/base/curves/k256/impl/)) and [big integer operations](./pkg/base/nt/numct/) (wraps [saferith](https://github.com/cronokirby/saferith) and [BoringSSL](./pkg/base/cgo/boring/)).

**Secure erasure**: Bron protocols are stateless and do not securely erase or invalidate secret material for callers. Applications integrating this library are responsible for the lifecycle and secure erasure of private keys, shares, witnesses, transcripts, serialized copies, backups, and any other secret state. This is especially important for share refresh, recovery, and redistribution: these protocols create new shares, but old shares may remain valid and security may be undermined unless the application reliably destroys every obsolete copy.
