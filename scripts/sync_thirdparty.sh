...
ALL_SYNCED=true
for dependency in $(find thirdparty -type d -mindepth 2 -maxdepth 2); do
    echo " --> Syncing $dependency"
    # read the config variables `dependencyRepository` & `dependencyCommit`
    if [ ! -e $dependency/.config ]; then
        echo " ---> Dependency config file not found: $dependency.config"
        ALL_SYNCED=false
    else 
        source $dependency/.config
        echo " ---> Ferching repository $dependencyRepository, commit $dependencyCommit"
