...
func testAliasing(t *testing.T, i int, x, y []byte, anyOverlap, inexactOverlap bool) {
	t.Helper()
	isThereOverlap := AnyOverlap(x, y)
	if isThereOverlap != anyOverlap {
		t.Errorf("%d: wrong AnyOverlap result, expected %v, got %v", i, anyOverlap, isThereOverlap)
	}
	inexact := InexactOverlap(x, y)
	if inexact != inexactOverlap {
		t.Errorf("%d: wrong InexactOverlap result, expected %v, got %v", i, inexactOverlap, isThereOverlap)
	}
}
