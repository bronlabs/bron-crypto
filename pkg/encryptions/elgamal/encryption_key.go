...
func NewKeys(curve curves.Curve, prng io.Reader) (*EncryptionKey, *DecryptionKey, error) {
	if curve == nil {
		return nil, nil, errs.NewIsNil("invalid curve")
	}
	x := curve.Scalar().Random(prng)
	if x == nil {
		return nil, nil, errs.NewIsZero("cannot generate decryption key")
	}
	value := curve.ScalarBaseMult(x)
	if value == nil {
		return nil, nil, errs.NewIsNil("cannot generate encryption key")
	}
...
func (ek *EncryptionKey) encryptWithRandNonce(msg []byte, msgIsHashed bool, r curves.Scalar, h curves.Point, nonce []byte) (*CipherText, error) {
	...
	if err != nil {
		return nil, errs.WrapFailed(err, "cannot create AEAD key")
	}
	block, err := aes.NewCipher(aeadKey.Bytes())
	if err != nil {
		return nil, errs.NewFailed("cannot create cipher")
	}
	aesGcm, err := cipher.NewGCM(block)
	if err != nil {
		return nil, errs.NewFailed("cannot create GCM")
	}
