...
func testPreviousDkgRoundReuse(t *testing.T, curve curves.Curve, hash func() hash.Hash, threshold, n int) {
	...
	victimIndex := 1
	require.NoError(t, err)
	cohortConfig, err := ttu.MakeThresholdProtocol(curve, identities, threshold)
	require.NoError(t, err)
	uniqueSessionId, err := agreeonrandom_testutils.RunAgreeOnRandom(curve, identities, crand.Reader)
	require.NoError(t, err)
	participants, err := testutils.MakeParticipants(uniqueSessionId, cohortConfig, identities, cn, nil)
	require.NoError(t, err)
...
func testAliceDlogProofIsUnique(t *testing.T, curve curves.Curve, hash func() hash.Hash, threshold, n int) {
	...
	alphaParticipants, err := testutils.MakeParticipants(alphaUniqueSessionId, cohortConfig, identities, cn, alphaPrngs)
	require.NoError(t, err)
	...
	betaParticipants, err := testutils.MakeParticipants(betaUniqueSessionId, cohortConfig, identities, cn, betaPrngs)
	require.NoError(t, err)
...
func testAbortOnRogueKeyAttach(t *testing.T, curve curves.Curve, hash func() hash.Hash) {
	...
	require.NoError(t, err)
	cohortConfig, err := ttu.MakeThresholdProtocol(curve, identities, 2)
	require.NoError(t, err)
	uniqueSessionId, err := agreeonrandom_testutils.RunAgreeOnRandom(curve, identities, crand.Reader)
	require.NoError(t, err)
...
func testPreviousDkgExecutionReuse(t *testing.T, curve curves.Curve, hash func() hash.Hash, tAlpha, nAlpha, tBeta, nBeta int) {
	...
	require.NoError(t, err)
	cohortConfigAlpha, err := ttu.MakeThresholdProtocol(curve, identities[:nAlpha], tAlpha)
	require.NoError(t, err)
	participantsAlpha, err := testutils.MakeParticipants(uniqueSessionIdAlpha, cohortConfigAlpha, identities[:nAlpha], cn, nil)
	require.NoError(t, err)
	...
	cohortConfigBeta, err := ttu.MakeThresholdProtocol(curve, identities[:nBeta], tBeta)
	require.NoError(t, err)
