...
func MapDkgRound1OutputsToRound2Inputs(participants []*gennaro.Participant, round1BroadcastOutputs []*gennaro.Round1Broadcast, round1UnicastOutputs []map[types.IdentityHash]*gennaro.Round1P2P) (round2BroadcastInputs []map[types.IdentityHash]*gennaro.Round1Broadcast, round2UnicastInputs []map[types.IdentityHash]*gennaro.Round1P2P) {
	round2BroadcastInputs = make([]map[types.IdentityHash]*gennaro.Round1Broadcast, len(participants))
	for i := range participants {
		round2BroadcastInputs[i] = make(map[types.IdentityHash]*gennaro.Round1Broadcast)
	...
	round2UnicastInputs = make([]map[types.IdentityHash]*gennaro.Round1P2P, len(participants))
	for i := range participants {
		round2UnicastInputs[i] = make(map[types.IdentityHash]*gennaro.Round1P2P)
		for j := range participants {
			if j != i {
...
func MapDkgRound2OutputsToRound3Inputs(participants []*gennaro.Participant, round3Outputs []*gennaro.Round2Broadcast) (round3Inputs []map[types.IdentityHash]*gennaro.Round2Broadcast) {
	round3Inputs = make([]map[types.IdentityHash]*gennaro.Round2Broadcast, len(participants))
	for i := range participants {
		round3Inputs[i] = make(map[types.IdentityHash]*gennaro.Round2Broadcast)
		for j := range participants {
			if j != i {
