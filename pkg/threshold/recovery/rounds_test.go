...
func Test_HappyPathWithNewIdentity(t *testing.T) {
	...
	// pretend bob lost his share and identity
	aliceShare := shares[0]
	alicePublicShare := publicShares[0]
	charlieShare := shares[2]
	charliePublicShare := publicShares[2]
	var newIdentities []types.IdentityKey
	for newIdentities == nil || !newIdentities[0].Equal(identities[0]) || !newIdentities[2].Equal(identities[2]) {
		newIdentities = make([]types.IdentityKey, n)
		bob, err := testutils.MakeDeterministicTestIdentities(1)
		require.NoError(t, err)

		newIdentities[0] = identities[0]
		newIdentities[1] = bob[0]
