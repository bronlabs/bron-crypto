...
func DoRecoveryRound1(participants []*recovery.Participant) (round1BroadcastOutputs []*recovery.Round1Broadcast, round1UnicastOutputs []network.RoundMessages[*recovery.Round1P2P], err error) {
	round1BroadcastOutputs = make([]*recovery.Round1Broadcast, len(participants))
	round1UnicastOutputs = make([]network.RoundMessages[*recovery.Round1P2P], len(participants))
...
func DoRecoveryRound2(participants []*recovery.Participant, lostPartyIndex int, round2BroadcastInputs []network.RoundMessages[*recovery.Round1Broadcast], round2UnicastInputs []network.RoundMessages[*recovery.Round1P2P]) (round2p2p []network.RoundMessages[*recovery.Round2P2P], err error) {
	round2p2p = make([]network.RoundMessages[*recovery.Round2P2P], len(participants))
	lastRecorded := 0
	for i, participant := range participants {
...
func DoRecoveryRound3(participants []*recovery.Participant, lostPartyIndex int, round3UnicastInputs network.RoundMessages[*recovery.Round2P2P]) (signingKeyShare *tsignatures.SigningKeyShare, err error) {
