...
func (p *Cosigner[V]) Round2(broadcastInput types.RoundMessages[*Round1Broadcast]) (broadcastOutput *Round2Broadcast, err error) {
	if p.round != 2 {
...
func (p *Cosigner[V]) Round3(broadcastInput types.RoundMessages[*Round2Broadcast], message []byte) (partialSignature *tschnorr.PartialSignature, err error) {
	if p.round != 3 {
	...
	e, err := schnorr.MakeGenericSchnorrChallenge(p.protocol.CipherSuite(), eBytes)
	if err != nil {
