...
func Keygen[C curves.Curve[P, F, S], P curves.Point[P, F, S], F fields.FiniteFieldElement[F], S fields.PrimeFieldElement[S]](protocol types.ThresholdProtocol[C, P, F, S], prng io.Reader) (ds.Map[types.IdentityKey, *lindell22.Shard[C, P, F, S]], error) {
	...
	shards := hashmap.NewHashableHashMap[types.IdentityKey, *lindell22.Shard[C, P, F, S]]()
	...
	for _, identityKey := range sharingConfig.Iter() {
		...
		shards.Put(identityKey, &lindell22.Shard[C, P, F, S]{
			SigningKeyShare: sks,
			PublicKeyShares: ppk,
		})
	}
