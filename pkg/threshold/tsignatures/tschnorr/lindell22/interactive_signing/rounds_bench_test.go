...
func Benchmark_InteractiveSigning(b *testing.B) {
	...
	identities, err := testutils2.MakeTestIdentities(signatureProtocol, n)
	require.NoError(b, err)

	thresholdProtocol, err := testutils2.MakeThresholdSignatureProtocol(signatureProtocol, identities, th, identities)
	require.NoError(b, err)

	shards, err := trusted_dealer.Keygen(thresholdProtocol, prng)
	require.NoError(b, err)

	transcripts := testutils2.MakeTranscripts("Lindell 2022 Interactive Sign", identities)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		participants, err := testutils.MakeParticipants(sid, thresholdProtocol, identities[:th], shards, transcripts, false)
		require.NoError(b, err)
