...
func (c *Cosigner) ProducePartialSignature(message []byte) (partialSignature *lindell22.PartialSignature, err error) {
	...
	var e curves.Scalar
	if c.taproot {
		e, err = hashing.HashToSchnorrScalar(c.cohortConfig.CipherSuite, bigR.ToAffineCompressed()[1:], c.myShard.SigningKeyShare.PublicKey.ToAffineCompressed()[1:], message)
	} else {
		e, err = hashing.HashToSchnorrScalar(c.cohortConfig.CipherSuite, bigR.ToAffineCompressed(), c.myShard.SigningKeyShare.PublicKey.ToAffineCompressed(), message)
	}
	if err != nil {
	...
	dPrime, err := interactive_signing.ToAdditiveShare(c.myShard.SigningKeyShare.Share, c.mySharingId, c.sessionParticipants, c.identityKeyToSharingId)
	if err != nil {
