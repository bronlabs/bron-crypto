...
func Aggregate(publicShard *PublicShard, partialSignatures ...*PartialSignature) (*saferith.Nat, error) {
	dealer1 := rep23.NewIntExpScheme(publicShard.N1)
	s1Shares := sliceutils.Map(partialSignatures, func(s *PartialSignature) *rep23.IntExpShare { return s.S1Share })
	...
	s2Shares := sliceutils.Map(partialSignatures, func(s *PartialSignature) *rep23.IntExpShare { return s.S2Share })
