...
// 	for i := 0; i < p.tau; i++ {
// 		k[i], err = p.protocol.Curve().ScalarField().Random(p.prng)
// 		if err != nil {
...
// 		for _, identity := range p.protocol.Participants().Iter() {
...
// func (p *PreGenParticipant) Round3(input types.RoundMessages[*Round2Broadcast]) (preSignatureBatch *lindell17.PreSignatureBatch, err error) {
// 	if p.round != 3 {
