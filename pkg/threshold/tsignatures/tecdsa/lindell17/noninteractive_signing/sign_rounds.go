...
func (p *Cosigner) ProducePartialSignature(message []byte) (partialSignature *lindell17.PartialSignature, err error) {
	...
	theirLambda, err := interactive_signing.CalcOtherPartyLagrangeCoefficient(p.theirSharingId, p.mySharingId, p.cohortConfig.Protocol.TotalParties, p.cohortConfig.CipherSuite.Curve)
	if err != nil {
