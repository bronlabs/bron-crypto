...
func testHappyPathWithParallelParties(t *testing.T, curve curves.Curve, h func() hash.Hash, threshold int, n int) {
	...
	participants, shards, err := dkls23_testutils.RunDKGWithParallelParties(curve, protocol, identities)
	require.NoError(t, err)
	require.NotNil(t, shards)
	for _, shard := range shards {
		require.NotNil(t, shard.SigningKeyShare)
...
func testHappyPathRunner(t *testing.T, curve curves.Curve, h func() hash.Hash, threshold int, n int) {
	...
	participants, err := dkls23_testutils.MakeDkgParticipants(curve, protocol, identities, signingKeyShares, partialPublicKeys, nil, nil)
	require.NoError(t, err)
	...
	go func() {
		...
		for i, party := range participants {
			errGrp.Go(func() error {
				var err error
				shards[i], err = party.Run(router, signingKeyShares[i])
				return err
			})
		}
		errChan <- errGrp.Wait()
