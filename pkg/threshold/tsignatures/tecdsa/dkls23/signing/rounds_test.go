...
func testHappyPath(t *testing.T, protocol protocols.Protocol, curve curves.Curve, h func() hash.Hash, threshold, n int, message []byte) {
	...
	for _, combinationIndices := range combinations {
		...
		t.Run(fmt.Sprintf("running the happy path with identities %v", identities), func(t *testing.T) {
			t.Parallel()
			err := testutils.RunInteractiveSign(cohortConfig, identities, selectedShards, message, seededPrng, nil)
			require.NoError(t, err)
		})
	}
}
...
func testFailForReplayedMessages(t *testing.T, protocol protocols.Protocol, curve curves.Curve, h func() hash.Hash, threshold, n int, message []byte) {
	t.Helper()
	...
	t.Run(fmt.Sprintf("running the replayed messages unhappy path with identities %v", identities), func(t *testing.T) {
		...
		participants, err := testutils.MakeInteractiveCosigners(cohortConfig, identities, shards, nil, seededPrng, nil)
		require.NoError(t, err)
		...
		participants2, err := testutils.MakeInteractiveCosigners(cohortConfig, identities, shards, nil, seededPrng, nil)
		require.NoError(t, err)
		...
		participants3, err := testutils.MakeInteractiveCosigners(cohortConfig, identities, shards, nil, seededPrng, nil)
		require.NoError(t, err)
