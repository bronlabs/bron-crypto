...
import (
	...
	"github.com/copperexchange/knox-primitives/pkg/threshold/tsignatures/tecdsa/dkls23/signing/interactive"
)
...
func DoInteractiveSignRound1(participants []*interactive.Cosigner) (round1OutputsBroadcast []*interactive.Round1Broadcast, round1OutputsP2P []map[helper_types.IdentityHash]*interactive.Round1P2P, err error) {
	round1OutputsBroadcast = make([]*interactive.Round1Broadcast, len(participants))
	round1OutputsP2P = make([]map[helper_types.IdentityHash]*interactive.Round1P2P, len(participants))
...
func MapInteractiveSignRound1OutputsToRound2Inputs(participants []*interactive.Cosigner, round1BroadcastOutputs []*interactive.Round1Broadcast, round1UnicastOutputs []map[helper_types.IdentityHash]*interactive.Round1P2P) (round2BroadcastInputs []map[helper_types.IdentityHash]*interactive.Round1Broadcast, round2UnicastInputs []map[helper_types.IdentityHash]*interactive.Round1P2P) {
	round2BroadcastInputs = make([]map[helper_types.IdentityHash]*interactive.Round1Broadcast, len(participants))
	for i := range participants {
		round2BroadcastInputs[i] = make(map[helper_types.IdentityHash]*interactive.Round1Broadcast)
	...
	round2UnicastInputs = make([]map[helper_types.IdentityHash]*interactive.Round1P2P, len(participants))
	for i := range participants {
		round2UnicastInputs[i] = make(map[helper_types.IdentityHash]*interactive.Round1P2P)
		for j := range participants {
			if j != i {
...
func DoInteractiveSignRound2(participants []*interactive.Cosigner, round2BroadcastInputs []map[helper_types.IdentityHash]*interactive.Round1Broadcast, round2UnicastInputs []map[helper_types.IdentityHash]*interactive.Round1P2P) (round2BroadcastOutputs []*interactive.Round2Broadcast, round2UnicastOutputs []map[helper_types.IdentityHash]*interactive.Round2P2P, err error) {
	round2BroadcastOutputs = make([]*interactive.Round2Broadcast, len(participants))
	round2UnicastOutputs = make([]map[helper_types.IdentityHash]*interactive.Round2P2P, len(participants))
	for i := range participants {
...
func MapInteractiveSignRound2OutputsToRound3Inputs(participants []*interactive.Cosigner, round2BroadcastOutputs []*interactive.Round2Broadcast, round2UnicastOutputs []map[helper_types.IdentityHash]*interactive.Round2P2P) (round3BroadcastInputs []map[helper_types.IdentityHash]*interactive.Round2Broadcast, round3UnicastInputs []map[helper_types.IdentityHash]*interactive.Round2P2P) {
	round3BroadcastInputs = make([]map[helper_types.IdentityHash]*interactive.Round2Broadcast, len(participants))
	for i := range participants {
		round3BroadcastInputs[i] = make(map[helper_types.IdentityHash]*interactive.Round2Broadcast)
		for j := range participants {
			if j != i {
	...
	round3UnicastInputs = make([]map[helper_types.IdentityHash]*interactive.Round2P2P, len(participants))
	for i := range participants {
		round3UnicastInputs[i] = make(map[helper_types.IdentityHash]*interactive.Round2P2P)
		for j := range participants {
			if j != i {
...
func DoInteractiveSignRound3(participants []*interactive.Cosigner, round3BroadcastInputs []map[helper_types.IdentityHash]*interactive.Round2Broadcast, round3UnicastInputs []map[helper_types.IdentityHash]*interactive.Round2P2P, message []byte) (partialSignatures []*dkls23.PartialSignature, err error) {
...
func MapPartialSignatures(identities []integration.IdentityKey, partialSignatures []*dkls23.PartialSignature) map[helper_types.IdentityHash]*dkls23.PartialSignature {
	result := make(map[helper_types.IdentityHash]*dkls23.PartialSignature)
	for i, identity := range identities {
