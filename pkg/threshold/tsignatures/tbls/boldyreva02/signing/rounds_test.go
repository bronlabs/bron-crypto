	...
	for _, config := range []struct {
		...
		t.Run(fmt.Sprintf("running happy path for t=%d and n=%d", boundedConfig.threshold, boundedConfig.total), func(t *testing.T) {
			...
			t.Run("short keys", func(t *testing.T) {
				t.Parallel()
				err := testutils.SigningRoundTrip[bls.G1, bls.G2](boundedConfig.threshold, boundedConfig.total)
				require.NoError(t, err)
			})
			t.Run("short signatures", func(t *testing.T) {
				t.Parallel()
				err := testutils.SigningRoundTrip[bls.G2, bls.G1](boundedConfig.threshold, boundedConfig.total)
				require.NoError(t, err)
			})
		})
	}
