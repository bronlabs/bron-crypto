...
import (
	...
	gentu "github.com/bronlabs/bron-crypto/pkg/threshold/sharing/interactive/dkg/gennaro/testutils"
...
func DoLindell22Round1[
	...
	for _, pi := range participants {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("%d could not run lindell22 signing round 1", pi.SharingID())
		}
	}
...
func DoLindell22Round2[
	...
	for _, pi := range participants {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("%d could not run lindell22 signing round 2", pi.SharingID())
		}
	}
	return r2bo, nil
}
