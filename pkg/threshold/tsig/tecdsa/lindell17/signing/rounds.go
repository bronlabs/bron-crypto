...
func (sc *SecondaryCosigner[P, B, S]) Round4(r3out *Round3OutputP2P[P, B, S], message []byte) (round4Output *lindell17.PartialSignature, err error) {
	...
	if !exists {
		return nil, ErrFailed.WithMessage("couldn't get primary paillier public key")
	}
	cKey, exists := sc.shard.EncryptedShares().Get(sc.primarySharingId)
	if !exists {
		return nil, ErrFailed.WithMessage("couldn't get primary encrypted signing key share")
	}
	...
	if !exists {
		return nil, ErrFailed.WithMessage("could not get primary Lagrange coefficient")
	}
