...
func DealRandom[P curves.Point[P, B, S], B algebra.PrimeFieldElement[B], S algebra.PrimeFieldElement[S]](curve ecdsa.Curve[P, B, S], shareholder ds.Set[sharing.ID], prng io.Reader) (ds.Map[sharing.ID, *lindell17.Shard[P, B, S]], *ecdsa.PublicKey[P, B, S], error) {
	...
	if err != nil {
		return nil, nil, errs.WrapFailed(err, "cannot create ecdsa public key")
	}
	...
	for id, share := range feldmanOutput.Shares().Iter() {
		...
		if err != nil {
			return nil, nil, errs.WrapRandomSample(err, "cannot generate paillier keypair")
		}
	...
	for id := range feldmanOutput.Shares().Iter() {
		...
		if !exists {
			return nil, nil, errs.WrapFailed(err, "cannot get share for shareholder %d", id)
		}
		baseShard, err := tecdsa.NewShard(share, feldmanOutput.VerificationMaterial(), feldmanDealer.AccessStructure())
		if err != nil {
			return nil, nil, errs.WrapFailed(err, "cannot create tECDSA Lindell17 shard")
		}

		shards[id], err = lindell17.NewShard(baseShard, auxInfo)
		if err != nil {
			return nil, nil, errs.WrapFailed(err, "cannot create lindell17 shard")
		}
	}
