...
func Test_HappyPath(t *testing.T) {
	...
	cosigners := make([]*sign_softspoken.Cosigner[*p256.Point, *p256.BaseFieldElement, *p256.Scalar], THRESHOLD)
	c1Shard, ok := shards.Get(sharing.ID(1))
	require.True(t, ok)
	c1, err := sign_softspoken.NewCosigner(sessionId, sharing.ID(1), quorum, suite, c1Shard, prng, c1Transcript)
	require.NoError(t, err)
	c2Shard, ok := shards.Get(sharing.ID(2))
	require.True(t, ok)
	c2, err := sign_softspoken.NewCosigner(sessionId, sharing.ID(2), quorum, suite, c2Shard, prng, c2Transcript)
	require.NoError(t, err)
	cosigners[0] = c1
	cosigners[1] = c2
	...
	r2bo := make(map[sharing.ID]*sign_softspoken.Round2Broadcast[*p256.Point, *p256.BaseFieldElement, *p256.Scalar])
	r2uo := make(map[sharing.ID]ds.Map[sharing.ID, *sign_softspoken.Round2P2P[*p256.Point, *p256.BaseFieldElement, *p256.Scalar]])
	for _, cosigner := range cosigners {
	...
	signature, err := dkls23.Aggregate(suite, pk, slices.Collect(maps.Values(partialSignatures))...)
	require.NoError(t, err)
