...
func RunDKLs23SignSoftspokenOT[P curves.Point[P, B, S], B algebra.FieldElement[B], S algebra.PrimeFieldElement[S]](tb testing.TB, shards map[sharing.ID]*tecdsa.Shard[P, B, S], quorum ds.Set[sharing.ID], message []byte, hashFunc func() hash.Hash) *ecdsa.Signature[S] {
	tb.Helper()
	...
	curve := algebra.StructureMustBeAs[curves.Curve[P, B, S]](pk.Structure())
	...
	for id := range quorum.Iter() {
		...
		consignersMap[id], err = sign_softspoken.NewCosigner(sessionId, id, quorum, ecdsaSuite, shard, prng, tapesMap[id])
		require.NoError(tb, err)
	}
