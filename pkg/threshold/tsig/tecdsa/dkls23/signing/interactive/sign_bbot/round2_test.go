...
func Test_HappyPath(t *testing.T) {
	...
	require.True(t, ok)
	c1, err := sign_bbot.NewCosigner(sessionId, sharing.ID(1), quorum, suite, c1Shard, prng, transcript.Clone())
	require.NoError(t, err)
	c2Shard, ok := shards.Get(sharing.ID(2))
	require.True(t, ok)
	c2, err := sign_bbot.NewCosigner(sessionId, sharing.ID(2), quorum, suite, c2Shard, prng, transcript.Clone())
	require.NoError(t, err)
	cosigners[0] = c1
	cosigners[1] = c2
