...
func TestCosignerCreationErrors(t *testing.T) {
	...
	shards, err := tu.DoBoldyrevaDKG(t, parties.Values(), true)
	require.NoError(t, err)
	shard, _ := shards.Get(sharing.ID(0))

	quorumSet := hashset.NewComparable[sharing.ID]()
	quorumSet.Add(sharing.ID(0))
...
func TestDifferentQuorumConfigurations(t *testing.T) {
	...
	testCases := []struct {
		...
		{"MinimalQuorum_2of3", 3, 2, []sharing.ID{0, 1}},
		{"ExactThreshold_3of5", 5, 3, []sharing.ID{0, 2, 4}},
		{"OverThreshold_4of5", 5, 3, []sharing.ID{0, 1, 2, 3}},
		{"AllParticipants_5of5", 5, 5, []sharing.ID{0, 1, 2, 3, 4}},
	}
