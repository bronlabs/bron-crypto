package pedersen_vss_test

import (
	crand "crypto/rand"
	"fmt"
	"github.com/bronlabs/bron-crypto/pkg/base/algebra/fields"
	"github.com/bronlabs/bron-crypto/pkg/base/curves/bls12381"
	"github.com/bronlabs/bron-crypto/pkg/base/curves/edwards25519"
	"github.com/bronlabs/bron-crypto/pkg/base/curves/p256"
	"github.com/bronlabs/bron-crypto/pkg/base/curves/pasta"
	"github.com/bronlabs/bron-crypto/pkg/base/types"
	"github.com/bronlabs/bron-crypto/pkg/base/utils/maputils"
	"maps"
	"slices"
	"testing"

	"github.com/stretchr/testify/require"

	"github.com/bronlabs/bron-crypto/pkg/base/curves"
	"github.com/bronlabs/bron-crypto/pkg/base/curves/k256"
	ecpedersen_comm "github.com/bronlabs/bron-crypto/pkg/commitments/pedersen"
	ecpedersen_vss "github.com/bronlabs/bron-crypto/pkg/threshold/sharing/pedersen"
)

var supportedAccessStructures = []struct{ th, n uint }{
	{2, 2},
	{2, 3},
	{3, 3},
	{3, 5},
	{7, 18},
}

func TestPedersenHappyPath(t *testing.T) {
	t.Parallel()
	for _, as := range supportedAccessStructures {
		t.Run(fmt.Sprintf("(%d/%d)", as.th, as.n), func(t *testing.T) {
			t.Parallel()
			t.Run("k256", func(t *testing.T) {
				t.Parallel()
				curve := k256.NewCurve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
			t.Run("p256", func(t *testing.T) {
				t.Parallel()
				curve := p256.NewCurve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
			t.Run("edwards25519", func(t *testing.T) {
				t.Parallel()
				curve := edwards25519.NewCurve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
			t.Run("pallas", func(t *testing.T) {
				t.Parallel()
				curve := pasta.NewPallasCurve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
			t.Run("vesta", func(t *testing.T) {
				t.Parallel()
				curve := pasta.NewVestaCurve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
			t.Run("bls12381g1", func(t *testing.T) {
				t.Parallel()
				curve := bls12381.NewG1Curve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
			t.Run("bls12381g2", func(t *testing.T) {
				t.Parallel()
				curve := bls12381.NewG2Curve()
				testPedersenHappyPath(t, curve, as.th, as.n)
			})
		})
	}
}

func TestPedersenLinearAdd(t *testing.T) {
	t.Parallel()
	for _, as := range supportedAccessStructures {
		t.Run(fmt.Sprintf("(%d/%d)", as.th, as.n), func(t *testing.T) {
			t.Parallel()
			t.Run("k256", func(t *testing.T) {
				t.Parallel()
				curve := k256.NewCurve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
			t.Run("p256", func(t *testing.T) {
				t.Parallel()
				curve := p256.NewCurve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
			t.Run("edwards25519", func(t *testing.T) {
				t.Parallel()
				curve := edwards25519.NewCurve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
			t.Run("pallas", func(t *testing.T) {
				t.Parallel()
				curve := pasta.NewPallasCurve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
			t.Run("vesta", func(t *testing.T) {
				t.Parallel()
				curve := pasta.NewVestaCurve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
			t.Run("bls12381g1", func(t *testing.T) {
				t.Parallel()
				curve := bls12381.NewG1Curve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
			t.Run("bls12381g2", func(t *testing.T) {
				t.Parallel()
				curve := bls12381.NewG2Curve()
				testPedersenLinearAdd(t, curve, as.th, as.n)
			})
		})
	}
}

func testPedersenHappyPath[C curves.Curve[P, F, S], P curves.Point[P, F, S], F fields.FiniteFieldElement[F], S fields.PrimeFieldElement[S]](t *testing.T, curve C, th, n uint) {
	t.Helper()
	prng := crand.Reader

	h, err := curve.Random(prng)
	require.NoError(t, err)
	ck := ecpedersen_comm.NewCommittingKey(curve.Generator(), h)

	scheme, err := ecpedersen_vss.NewScheme(curve, ck, th, n)
	require.NoError(t, err)

	randomScalar, err := curve.ScalarField().Random(prng)
	require.NoError(t, err)

	shares, commitmentVector, err := scheme.DealVerifiable(randomScalar, prng)
	require.NoError(t, err)
	require.NotNil(t, shares)
	require.Len(t, commitmentVector, int(th))
	for _, share := range shares {
		err = scheme.VerifyShare(share, commitmentVector)
		require.NoError(t, err)
	}

	secret, err := scheme.Open(slices.Collect(maps.Values(shares))...)
	require.NoError(t, err)
	require.True(t, secret.Equal(randomScalar))

}

// TODO(mkk): test other linear ops
func testPedersenLinearAdd[C curves.Curve[P, F, S], P curves.Point[P, F, S], F fields.FiniteFieldElement[F], S fields.PrimeFieldElement[S]](t *testing.T, curve C, th, n uint) {
	t.Helper()
	prng := crand.Reader

	h, err := curve.Random(prng)
	require.NoError(t, err)
	ck := ecpedersen_comm.NewCommittingKey(curve.Generator(), h)

	scheme, err := ecpedersen_vss.NewScheme(curve, ck, th, n)
	require.NoError(t, err)

	randomScalarA, err := curve.ScalarField().Random(prng)
	require.NoError(t, err)
	randomScalarB, err := curve.ScalarField().Random(prng)
	require.NoError(t, err)

	randomScalar := randomScalarA.Add(randomScalarB)

	sharesA, commitmentVectorA, err := scheme.DealVerifiable(randomScalarA, prng)
	require.NoError(t, err)
	require.NotNil(t, sharesA)
	require.Len(t, commitmentVectorA, int(th))

	sharesB, commitmentVectorB, err := scheme.DealVerifiable(randomScalarB, prng)
	require.NoError(t, err)
	require.NotNil(t, sharesB)
	require.Len(t, commitmentVectorB, int(th))

	shares := maputils.Join(sharesA, sharesB, func(_ types.SharingID, l, r *ecpedersen_vss.Share[S]) *ecpedersen_vss.Share[S] {
		return scheme.ShareAdd(l, r)
	})
	commitmentVector := scheme.VerificationAdd(commitmentVectorA, commitmentVectorB)

	for _, share := range shares {
		err = scheme.VerifyShare(share, commitmentVector)
		require.NoError(t, err)
	}

	secret, err := scheme.Open(slices.Collect(maps.Values(shares))...)
	require.NoError(t, err)
	require.True(t, secret.Equal(randomScalar))

}
