...
func DoRound1(participants []*hjky.Participant) (round1BroadcastOutputs []*hjky.Round1Broadcast, round1UnicastOutputs []network.RoundMessages[*hjky.Round1P2P], err error) {
	round1BroadcastOutputs = make([]*hjky.Round1Broadcast, len(participants))
	round1UnicastOutputs = make([]network.RoundMessages[*hjky.Round1P2P], len(participants))
...
func DoDkgRound2(participants []*hjky.Participant, round2BroadcastInputs []network.RoundMessages[*hjky.Round1Broadcast], round2UnicastInputs []network.RoundMessages[*hjky.Round1P2P]) (samples []hjky.Sample, publicKeySharesMaps []ds.Map[types.IdentityKey, curves.Point], feldmanCommitmentVectors [][]curves.Point, err error) {
