...
func testHappyPathWithParallelParties(t *testing.T, curve curves.Curve, n int) {
	t.Helper()
	...
	r1OutsU, err := testutils.DoSetupRound1WithParallelParties(participants)
	require.NoError(t, err)
	for _, out := range r1OutsU {
		require.Equal(t, out.Size(), len(identities)-1)
	}
	...
	r2InsU := ttu.MapUnicastO2I(participants, r1OutsU)
	r2OutsU, err := testutils.DoSetupRound2WithParallelParties(participants, r2InsU)
	require.NoError(t, err)
	for _, out := range r2OutsU {
		require.Equal(t, out.Size(), len(identities)-1)
	}
	...
	r3InsU := ttu.MapUnicastO2I(participants, r2OutsU)
	allPairwiseSeeds, err := testutils.DoSetupRound3WithParallelParties(participants, r3InsU)
	require.NoError(t, err)
