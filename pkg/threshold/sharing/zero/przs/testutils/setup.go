...
func MapSetupRound1OutputsToRound2Inputs(participants []*setup.Participant, round2Outputs []map[types.IdentityHash]*setup.Round1P2P) (round3Inputs []map[types.IdentityHash]*setup.Round1P2P) {
	round3Inputs = make([]map[types.IdentityHash]*setup.Round1P2P, len(participants))
	for i := range participants {
		round3Inputs[i] = make(map[types.IdentityHash]*setup.Round1P2P)
...
func MapSetupRound2OutputsToRound3Inputs(participants []*setup.Participant, round3Outputs []map[types.IdentityHash]*setup.Round2P2P) (round4Inputs []map[types.IdentityHash]*setup.Round2P2P) {
	round4Inputs = make([]map[types.IdentityHash]*setup.Round2P2P, len(participants))
	for i := range participants {
		round4Inputs[i] = make(map[types.IdentityHash]*setup.Round2P2P)
		for j := range participants {
			if j != i {
