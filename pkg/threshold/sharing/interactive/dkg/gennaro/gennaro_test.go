...
import (
	...
	tu "github.com/bronlabs/bron-crypto/pkg/threshold/sharing/interactive/dkg/gennaro/testutils"
...
func TestDKGShareProperties(t *testing.T) {
	...
	t.Run("share reconstruction subsets", func(t *testing.T) {
		t.Parallel()
		feldmanAS, err := accessstructures.NewThresholdAccessStructure(threshold, parties.Values()[0].AccessStructure().Shareholders())
		require.NoError(t, err)
...
func TestParticipantCreation(t *testing.T) {
	...
	t.Run("nil group", func(t *testing.T) {
		...
		ac, err := accessstructures.NewThresholdAccessStructure(3, shareholders)
		require.NoError(t, err)
