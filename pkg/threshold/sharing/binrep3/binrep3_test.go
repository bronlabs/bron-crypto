...
func Test_PseudoHomomorphicReverseBytes(t *testing.T) {
	prng := crand.Reader
	shareholders := sharing.NewOrdinalShareholderSet(3)
	scheme, err := NewScheme(shareholders)
	require.NoError(t, err)
	...
	secret3 := bits.ReverseBytes64(secret1)
	var shares3 []*Share
	for id := range shareholders.Iter() {
		l, _ := dealerOutput1.Shares().Get(id)
		shares3 = append(shares3, l.ReverseBytes())
	}
...
func Test_PseudoHomomorphicBitMask(t *testing.T) {
	prng := crand.Reader
	shareholders := sharing.NewOrdinalShareholderSet(3)
	scheme, err := NewScheme(shareholders)
	require.NoError(t, err)
	...
	secret3 := -((secret1 >> k) & 0b1)
	var shares3 []*Share
	for id := range shareholders.Iter() {
		l, _ := dealerOutput1.Shares().Get(id)
		shares3 = append(shares3, l.BitMaskOf(int(k)))
	}
