...
type Share struct {
	...
	p  uint64     `cbor:"prevShare"`
	n  uint64     `cbor:"nextShare"`
}
...
func (s *Share) ReverseBytes() *Share {
	return NewShare(s.id, bits.ReverseBytes64(s.p), bits.ReverseBytes64(s.n))
}

func (s *Share) BitMaskOf(bit int) *Share {
	return NewShare(s.id, -((s.p >> bit) & 0b1), -((s.n >> bit) & 0b1))
}
