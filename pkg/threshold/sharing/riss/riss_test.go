	...
	for _, as := range accessStructures {
		t.Run(fmt.Sprintf("(%d,%d)", as.th, as.n), func(t *testing.T) {
			...
			for range iters {
				...
				dealer, err := riss.NewDealer(as.th, as.n, riss.WithBitLen(bitLen), riss.WithSpecialForm(true))
				require.NoError(t, err)
				shares, err := dealer.Share(secret, prng)
				require.NoError(t, err)

				smallestSharingIdSet := riss.SharingIdSet((uint64(1) << (as.th - 1)) - 1)
				for sharingId, share := range shares {
					require.Equal(t, sharingId, share.SharingId())
	...
	for _, as := range accessStructures {
		t.Run(fmt.Sprintf("(%d,%d)", as.th, as.n), func(t *testing.T) {
			...
			for range iters {
				...
				require.NoError(t, err)
				shares := make(map[types.SharingID]*riss.IntShare)
