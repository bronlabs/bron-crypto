	...
	go func() {
		...
		for i, participant := range participants {
			...
			go func(i int, participant *agreeonrandom.Participant) {
				defer wg.Done()
				var err error
				round2BroadcastOutputs[i], err = participant.Round2(round2Inputs[i])
				if err != nil {
...
func DoRound3(participants []*agreeonrandom.Participant, round3Inputs []network.RoundMessages[types.Protocol, *agreeonrandom.Round2Broadcast]) (results [][]byte, err error) {
	...
	go func() {
		...
		for i, participant := range participants {
			...
			go func(i int, participant *agreeonrandom.Participant) {
				defer wg.Done()
				var err error
				r3Out[i], err = participant.Round3(round3Inputs[i])
				if err != nil {
