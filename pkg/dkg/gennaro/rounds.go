...
func (p *Participant) Round1() (*Round1Broadcast, map[integration.IdentityKey]*Round1P2P, error) {
	...
	proverTranscript.AppendMessage([]byte("sharing id"), []byte(fmt.Sprintf("%d", p.MySharingId)))
...
func (p *Participant) Round3(round2output map[integration.IdentityKey]*Round2Broadcast) (*threshold.SigningKeyShare, *threshold.PublicKeyShares, error) {
	...
	for senderSharingId := 1; senderSharingId <= p.CohortConfig.TotalParties; senderSharingId++ {
		...
		transcript.AppendMessage([]byte("sharing id"), []byte(fmt.Sprintf("%d", senderSharingId)))
