...
func DoRecoveryRound1(participants []*recovery.Participant) (round1BroadcastOutputs []*recovery.Round1Broadcast, round1UnicastOutputs []map[helper_types.IdentityHash]*recovery.Round1P2P, err error) {
	round1BroadcastOutputs = make([]*recovery.Round1Broadcast, len(participants))
	round1UnicastOutputs = make([]map[helper_types.IdentityHash]*recovery.Round1P2P, len(participants))
...
func MapRecoveryRound1OutputsToRound2Inputs(participants []*recovery.Participant, round1BroadcastOutputs []*recovery.Round1Broadcast, round1UnicastOutputs []map[helper_types.IdentityHash]*recovery.Round1P2P) (round2BroadcastInputs []map[helper_types.IdentityHash]*recovery.Round1Broadcast, round2UnicastInputs []map[helper_types.IdentityHash]*recovery.Round1P2P) {
	round2BroadcastInputs = make([]map[helper_types.IdentityHash]*recovery.Round1Broadcast, len(participants))
	for i := range participants {
		round2BroadcastInputs[i] = make(map[helper_types.IdentityHash]*recovery.Round1Broadcast)
	...
	round2UnicastInputs = make([]map[helper_types.IdentityHash]*recovery.Round1P2P, len(participants))
	for i := range participants {
		round2UnicastInputs[i] = make(map[helper_types.IdentityHash]*recovery.Round1P2P)
		for j := range participants {
			if j != i {
...
func DoRecoveryRound2(participants []*recovery.Participant, lostPartyIndex int, round2BroadcastInputs []map[helper_types.IdentityHash]*recovery.Round1Broadcast, round2UnicastInputs []map[helper_types.IdentityHash]*recovery.Round1P2P) (round2p2p []map[helper_types.IdentityHash]*recovery.Round2P2P, err error) {
	round2p2p = make([]map[helper_types.IdentityHash]*recovery.Round2P2P, len(participants))
	lastRecorded := 0
	for i, participant := range participants {
...
func MapRecoveryRound2OutputsToRound3Inputs(participants []*recovery.Participant, lostPartyIndex int, round2UnicastOutputs []map[helper_types.IdentityHash]*recovery.Round2P2P) (round3UnicastInputs map[helper_types.IdentityHash]*recovery.Round2P2P) {
	for i := range participants {
		if i != lostPartyIndex {
			continue
		}
		round3UnicastInputs = make(map[helper_types.IdentityHash]*recovery.Round2P2P)
		for j := range participants {
			if j != i {
...
func DoRecoveryRound3(participants []*recovery.Participant, lostPartyIndex int, round3UnicastInputs map[helper_types.IdentityHash]*recovery.Round2P2P) (signingKeyShare *threshold.SigningKeyShare, err error) {
