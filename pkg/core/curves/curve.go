...
type CurveProfile interface {
	// Fp, the finite field over which the curve is defined
	Field() FieldProfile
	// Order of the subgroup generated by G
	SubGroupOrder() *big.Int
	// h = Ord(Fp) / Ord(G) | For us, it's always the case that h < q hence the type being Scalar
	Cofactor() Scalar
	// returns the larger PairingCurve interface, or nil if the curve is not pairing friendly
	ToPairingCurve() PairingCurve
}

type PairingCurveProfile interface {
	EmbeddingDegree() *big.Int
}
