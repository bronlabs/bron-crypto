...
import (
	...
	rng "github.com/copperexchange/knox-primitives/pkg/core/prng"
)

const (
	// Test file names.
	File_drgbvectors_no_reseed = "test_utils/drbgtestvectors/drbgvectors_no_reseed/CTR_DRBG.rsp"
	File_drgbvectors_pr_false  = "test_utils/drbgtestvectors/drbgvectors_pr_false/CTR_DRBG.rsp"
	// Test file constants.
...
func (nistTest *NistTestHelper) RunInit(AesKeySize int) (rng.PRNG, error) {
	...
	prng, err := rng.NewAesPRNG(AesKeySize, nil, nistTest.State.EntropyInput, nistTest.State.Nonce, nistTest.State.PersonalizationStr)
	if err != nil {
...
func (nistTest *NistTestHelper) RunGenerate(prng rng.PRNG, buffer []byte) error {
...
func (nistTest *NistTestHelper) RunReseed(prng rng.PRNG, buffer []byte) error {
...
func (*NistTestHelper) RunRead(prng rng.PRNG, buffer []byte) error {
	...
	for nistTest.Scan() {
		if nistTest.IsNewTestCase() {
			...
			for nistTest.CountNo = 0; nistTest.CountNo < maxTestCount; nistTest.CountNo++ {
				...
				if err != nil {
					return errs.WrapFailed(err, "[line=%d] Error initialising test", nistTest.LineNo)
				}
				// Run reseed
				if withReseed {
					if err := nistTest.RunReseed(prng, returnedBits); err != nil {
						return errs.WrapFailed(err, "[line=%d] Error reseeding test", nistTest.LineNo)
					}
				}
				// Run generation
				if err := nistTest.RunGenerate(prng, returnedBits); err != nil {
					return errs.WrapFailed(err, "[line=%d] Error running test", nistTest.LineNo)
				}
				// Check test results
...
func RunNistValidationTest(keySize int, useDf bool) (err error) {
	for _, caseParams := range []struct {
		...
		if err = RunNistTestCases(f, keySize, useDf, caseParams.withReseed); err != nil {
			return errs.WrapFailed(err, "FAILED Nist test case with keySize=%d, useDf=%t, withReseed=%t ", keySize, useDf, caseParams.withReseed)
		}
