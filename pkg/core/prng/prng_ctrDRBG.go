...
import (
	...
	"github.com/copperexchange/knox-primitives/pkg/core/modular/uint128"
)
...
func (ctrDrbg *CtrDRBG) Update(providedData []byte) (err error) {
	...
	tempBlocks := modular.CeilDiv(ctrDrbg.SeedSize(), AesBlockSize)
...
func (ctrDrbg *CtrDRBG) Generate(outputBuffer, additionalInput []byte) (err error) {
	...
	requestedNumberOfBlocks := modular.CeilDiv(requestedNumberOfBytes, AesBlockSize)
...
func (ctrDrbg *CtrDRBG) BlockCipherDF(inputString []byte, noOfBytesToReturn int) (requestedBytes []byte, err error) {
	...
	sBlocks := modular.CeilDiv(int(4+4+l+1), AesBlockSize)
	...
	aesCipher, err := aes.NewCipher([]byte(initKey)[:ctrDrbg.keySize])
	if err != nil {
	...
	requestedBlocks := modular.CeilDiv(noOfBytesToReturn, AesBlockSize)
