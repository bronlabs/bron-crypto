...
const (
	...
	// initKey is the initial key used to initialise the PRNG.
	initKey = string(
...
// PRNG is a seeded Pseudo-Random Number Generator following NIST SP-800-90A spec.
type PRNG interface {
	io.Reader
	Generate(buffer, additionalInput []byte) error
	Reseed(entropyInput, additionalInput []byte) error
	SecurityStrength() int
}

// AesPRNG corresponds to an instantiated PRNG based on a block cipher from NIST SP-800-90A.
type AesPRNG struct {
...
// NewAesPRNG creates a PRNG as specified in SP-800-90A section 10.2. The PRNG uses
...
func NewAesPRNG(keySize int, entropySource io.Reader, entropyInput, nonce, personalization []byte) (prng PRNG, err error) {
	AesPrng := new(AesPRNG)
	...
	if entropySource != nil {
		AesPrng.entropySource = entropySource
	} else { // Default: use the OS-wide global, shared instance of a CS-RNG.
		AesPrng.entropySource = crand.Reader
	}
	...
	case entropyInputLen == 0: // Sample entropyInput if not provided
		...
		if _, err := AesPrng.entropySource.Read(entropyInput); err != nil {
	...
	case nonceLen == 0: // Sample nonce if not provided
		...
		if _, err = AesPrng.entropySource.Read(nonce); err != nil {
	...
	AesPrng.ctrDrbg = NewCtrDRBG(keySize)
	if err = AesPrng.ctrDrbg.Instantiate(entropyInput, nonce, personalization); err != nil {
...
func (prng *AesPRNG) Reseed(entropyInput, additionalInput []byte) (err error) {
	...
	// additional_input).
	if err = prng.ctrDrbg.Reseed(entropyInput, additionalInput); err != nil {
...
func (prng *AesPRNG) Generate(buffer, additionalInput []byte) error {
	if len(buffer) == 0 {
...
dataGeneration:
	...
	switch err := prng.ctrDrbg.Generate(buffer, additionalInput); {
	case errs.IsRandomSampleFailed(err) && prng.entropySource != nil:
	...
	if reseedRequired {
		...
		if err := prng.Reseed(nil, additionalInput); err != nil {
...
// error otherwise.
func (prng *AesPRNG) Read(buffer []byte) (n int, err error) {
	numRequests := modular.CeilDiv(len(buffer), maxNumberOfBytesRequest)
	for i := 0; i < numRequests; i++ {
		requestBuffer := buffer[i*maxNumberOfBytesRequest : (i+1)*maxNumberOfBytesRequest]
		if err := prng.Generate(nil, requestBuffer); err != nil {
...
// SecurityStrength returns the computational security parameter of this prng.
// Equates to the length of the key used in the internal AES block cipher.
func (prng *AesPRNG) SecurityStrength() int {
	return prng.ctrDrbg.keySize
}
