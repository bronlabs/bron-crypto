...
- `BatchAggregateVerify`: Simultaneously verifies n signatures, where each signature is itself an aggregate signature given a fixed set of public keys and some messages. The method is described [here](https://ethresear.ch/t/fast-verification-of-multiple-bls-signatures/5407)
- `BatchVerify`: Simultaneously verifies n different signatures. The trick works similar to above, except those signatures are not aggregated signatures. This is what the ETH test vectors test against, whilst claiming that they implement above.
...
- We deviate from one test vector: We do not allow aggregation of identity signatures, but the test vectors do.
