...
func TestCanSignAndVerify(t *testing.T) {
	...
	for _, scheme := range []bls.RogueKeyPrevention{
		...
		t.Run("short keys", func(t *testing.T) {
			t.Parallel()
			_, _, _, err := test_utils.RoundTripWithKeysInG1(message, boundedScheme)
			require.NoError(t, err)
		})

		t.Run("short signatures", func(t *testing.T) {
			t.Parallel()
			_, _, _, err := test_utils.RoundTripWithKeysInG2(message, boundedScheme)
			require.NoError(t, err)
		})
	}
}
...
func TestCannotVerify(t *testing.T) {
	...
	for _, scheme := range []bls.RogueKeyPrevention{
		...
		t.Run("random message", func(t *testing.T) {
			...
			t.Run("short keys", func(t *testing.T) {
				t.Parallel()
				privateKey, signature, pop, err := test_utils.RoundTripWithKeysInG1(message, boundedScheme)
				require.NoError(t, err)
			...
			t.Run("short signatures", func(t *testing.T) {
				t.Parallel()
				privateKey, signature, pop, err := test_utils.RoundTripWithKeysInG2(message, boundedScheme)
				require.NoError(t, err)
				err = bls.Verify(privateKey.PublicKey, signature, []byte("ETH > BTC"), pop, boundedScheme)
				require.Error(t, err)
			})
...
func TestCanSignAndVerifyInAggregate(t *testing.T) {
	...
	for _, scheme := range []bls.RogueKeyPrevention{
		...
		for _, batchSize := range []int{2, 5, 10} {
			...
			t.Run("short keys", func(t *testing.T) {
				t.Parallel()
				publicKeys := make([]*bls.PublicKey[test_utils.G1], boundedBatchSize)
				signatures := make([]*bls.Signature[test_utils.G2], boundedBatchSize)
				pops := make([]*bls.ProofOfPossession[test_utils.G2], boundedBatchSize)
			...
			t.Run("short signatures", func(t *testing.T) {
				t.Parallel()
				publicKeys := make([]*bls.PublicKey[test_utils.G2], boundedBatchSize)
				signatures := make([]*bls.Signature[test_utils.G1], boundedBatchSize)
				pops := make([]*bls.ProofOfPossession[test_utils.G1], boundedBatchSize)
				messages := make([][]byte, boundedBatchSize)
				...
				for i := 0; i < boundedBatchSize; i++ {
					...
					privateKey, signature, pop, err := test_utils.RoundTripWithKeysInG2(m, boundedScheme)
					require.NoError(t, err)
					publicKeys[i] = privateKey.PublicKey
					signatures[i] = signature
					pops[i] = pop
