...
func (ic *InteractiveCosigner) Round1() (*Round1Broadcast, error) {
	if ic.round != 1 {
		return nil, errors.Errorf("%s round mismatch %d != 1", error_types.EInvalidRound, ic.round)
	}
...
func (ic *InteractiveCosigner) Round2(round1output map[integration.IdentityKey]*Round1Broadcast, message []byte) (*frost.PartialSignature, error) {
	if ic.round != 2 {
		return nil, errors.Errorf("%s round mismatch %d != 2", error_types.EInvalidRound, ic.round)
	}
	D_alpha, E_alpha, err := ic.processNonceCommitmentOnline(round1output)
	if err != nil {
		return nil, errors.Wrapf(err, "%s couldn't not derive D alpha and E alpha", error_types.EAbort)
	}
...
func (ic *InteractiveCosigner) Aggregate(message []byte, partialSignatures map[integration.IdentityKey]*frost.PartialSignature) (*frost.Signature, error) {
	if ic.round != 3 {
		return nil, errors.Errorf("%s round mismatch %d != 3", error_types.EInvalidRound, ic.round)
	}
...
func (ic *InteractiveCosigner) processNonceCommitmentOnline(round1output map[integration.IdentityKey]*Round1Broadcast) (D_alpha, E_alpha map[integration.IdentityKey]curves.Point, err error) {
	...
	for _, senderIdentityKey := range ic.SessionParticipants {
		...
		if !exists {
			return nil, nil, errors.Errorf("%s sender identity key is not found", error_types.EMissing)
		}
		receivedMessage, exists := round1output[senderIdentityKey]
		if !exists {
			return nil, nil, errors.Errorf("%s do not have a message from shamir id %d", error_types.EMissing, shamirId)
		}
		D_i := receivedMessage.Di
		if D_i.IsIdentity() {
			return nil, nil, errors.Errorf("%s D_i of shamir id %d is at infinity", error_types.EMissing, shamirId)
		}
		if !D_i.IsOnCurve() {
			return nil, nil, errors.Errorf("%s D_i of shamir id %d is not on curve", error_types.EMissing, shamirId)
		}
		E_i := receivedMessage.Ei
		if E_i.IsIdentity() {
			return nil, nil, errors.Errorf("%s E_i of shamir id %d is at infinity", error_types.EIsIdentity, shamirId)
		}
		if !E_i.IsOnCurve() {
			return nil, nil, errors.Errorf("%s E_i of shamir id %d is not on curve", error_types.ENotOnCurve, shamirId)
		}
