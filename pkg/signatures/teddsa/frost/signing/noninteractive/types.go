...
func (ac *AttestedCommitmentToNoncePair) Validate(cohortConfig *integration.CohortConfig) error {
	if ac == nil {
		return errors.Errorf("%s attested commitment to nonce is nil", error_types.EIsNil)
	}
	if ac.Attestor == nil {
		return errors.Errorf("%s attestor is nil", error_types.EIsNil)
	}
	if !cohortConfig.IsInCohort(ac.Attestor) {
		return errors.Errorf("%s attestor is not in cohort", error_types.EInvalidArgument)
	}
	if ac.D.IsIdentity() {
		return errors.Errorf("%s D is at infinity", error_types.EIsIdentity)
	}
	if !ac.D.IsOnCurve() {
		return errors.Errorf("%s D is not on the curve", error_types.ENotOnCurve)
	}
	if ac.E.IsIdentity() {
		return errors.Errorf("%s E is at infinity", error_types.EIsIdentity)
	}
	if !ac.E.IsOnCurve() {
		return errors.Errorf("%s E is not on the curve", error_types.ENotOnCurve)
	}
	if ac.D.CurveName() != ac.E.CurveName() {
		return errors.Errorf("%s D and E are not on the same curve %s != %s", error_types.EInvalidCurve, ac.D.CurveName(), ac.E.CurveName())
	}
...
func (ps *PreSignature) Validate(cohortConfig *integration.CohortConfig) error {
	if ps == nil {
		return errors.Errorf("%s presignature is nil", error_types.EIsNil)
	}
	...
	for _, thisPartyAttestedCommitment := range *ps {
		if attestorsHashSet[thisPartyAttestedCommitment.Attestor] {
			return errors.Errorf("%s found duplicate attestor in this presignature", error_types.EDuplicate)
		}
		if DHashSet[thisPartyAttestedCommitment.D] {
			return errors.Errorf("%s found duplicate D", error_types.EDuplicate)
		}
		if EHashSet[thisPartyAttestedCommitment.E] {
			return errors.Errorf("%s found duplicate E", error_types.EDuplicate)
		}
		if err := thisPartyAttestedCommitment.Validate(cohortConfig); err != nil {
			return errors.Wrapf(err, "%s invalid attested commitments", error_types.EVerificationFailed)
		}
	...
	for _, participant := range cohortConfig.Participants {
		if !attestorsHashSet[participant] {
			return errors.Errorf("%s at least one party in the cohort does not have an attested commitment", error_types.EAbort)
		}
	}
	if err := sortPreSignatureInPlace(cohortConfig, *ps); err != nil {
		return errors.Wrapf(err, "%s couldn't sort presignature elements by shamir id", error_types.EAbort)
	}
	return nil
}
...
func (psb *PreSignatureBatch) Validate(cohortConfig *integration.CohortConfig) error {
	...
	if err := cohortConfig.Validate(); err != nil {
		return errors.Wrapf(err, "%s could not validate cohort config", error_types.EVerificationFailed)
	}
	if len(*psb) <= 0 {
		return errors.Errorf("%s batch is empty", error_types.EIsZero)
	}
	...
	for i, preSignature := range *psb {
		if err := preSignature.Validate(cohortConfig); err != nil {
			return errors.Wrapf(err, "%s presignature with index %d is invalid", error_types.EVerificationFailed, i)
		}
