...
func DeriveChildKeys[
	...
	if utils.IsNil(publicKey) {
		return *new(SH), nil, errs.NewFailed("public key is nil")
	}
	if i >= (1 << 31) {
		return *new(SH), nil, errs.NewFailed("unsupported derivation")
	}
	if publicKey.Value().Structure().Name() == k256.NewCurve().Name() {
		...
		if err != nil {
			return *new(SH), nil, errs.WrapFailed(err, "cannot derive child key")
		}
...
func bip32Like[
	...
	if !ok {
		return *new(SH), nil, errs.NewFailed("public key does not implement PrimeField")
	}
	...
	if err != nil {
		return *new(SH), nil, errs.WrapFailed(err, "cannot create blake2b xof")
	}

	if _, err := xof.Write(slices.Concat(publicKey.Value().Bytes(), binary.BigEndian.AppendUint32(nil, i))); err != nil {
		return *new(SH), nil, errs.WrapFailed(err, "cannot hash public key")
	}
	digest := make([]byte, digestLen)
	if _, err := io.ReadFull(xof, digest); err != nil {
		return *new(SH), nil, errs.WrapFailed(err, "cannot read digest")
	}

	childChainCode := digest[scalarWideLen:]
	shift, err := sf.FromWideBytes(digest[:scalarWideLen])
	if err != nil {
		return *new(SH), nil, errs.WrapFailed(err, "cannot create scalar from bytes")
	}
