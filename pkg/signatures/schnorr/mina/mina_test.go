...
func Test_MinaSignMessage(t *testing.T) {
	...
	pk := curve.Generator().ScalarMul(sk)
	...
	r := curve.Generator().ScalarMul(kPrime)
	...
	e, err := variant.ComputeChallenge(poseidon.NewLegacyHash, r, pk, message)
...
func strToScalar(t *testing.T, str string) *pasta.PallasScalar {
	...
	s, err := pasta.NewPallasScalarField().FromNat(n)
	require.NoError(t, err)
	return s
}
...
func testPrefix(t *testing.T, prefix mina.Prefix, expectedString string) {
	...
	expected, err := pasta.NewPallasBaseField().FromNat(expectedNat)
	require.NoError(t, err)
