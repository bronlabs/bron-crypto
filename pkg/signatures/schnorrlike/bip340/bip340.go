...
var (
	...
	_ mpcschnorr.MPCFriendlyScheme[*Variant, *GroupElement, *Scalar, Message, *KeyGenerator, *Signer, *Verifier] = (*Scheme)(nil)
...
func NewPublicKey(point *GroupElement) (*PublicKey, error) {
	...
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("failed to create BIP340 public key")
	}
	return pk, nil
}
...
func NewPrivateKey(scalar *Scalar) (*PrivateKey, error) {
	...
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("failed to create BIP340 private key")
	}
	return sk, nil
}
...
func NewScheme(prng io.Reader) (*Scheme, error) {
	...
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("cannot generate nonce")
	}
	return &Scheme{
		aux: aux,
	}, nil
...
func (v *Verifier) Verify(signature *Signature, publicKey *PublicKey, message Message) error {
	...
	if v.challengePublicKey != nil {
		...
		if err != nil {
			return errs.Wrap(err).WithMessage("cannot correct public key share parity")
		}
		bigP = correctedP
	...
	if v.challengePublicKey != nil && signature.E != nil {
		...
		if err != nil {
			return errs.Wrap(err).WithMessage("cannot create challenge scalar")
		}
	}
...
func (v *Verifier) BatchVerify(sigs []*Signature, publicKeys []*PublicKey, messages []Message) error {
	...
	for i := 1; i < len(sigs); i++ {
		...
		if err != nil {
			return errs.Wrap(err).WithMessage("cannot generate random scalar for i=%d", i)
		}
	}
	...
	for i, sig := range sigs {
		...
		if err != nil {
			return errs.Wrap(err).WithMessage("invalid signature")
		}
	...
	if err != nil {
		return errs.Wrap(err).WithMessage("failed to multiply scalars and points")
	}
