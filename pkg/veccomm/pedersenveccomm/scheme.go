...
func (*vectorHomomorphicScheme) CombineCommitments(x *VectorCommitment, ys ...*VectorCommitment) (*VectorCommitment, error) {
	if err := x.Validate(); err != nil {
		return nil, errs.WrapValidation(err, "invalid commitment (1st operand)")
	}
	...
	for _, y := range ys {
		if err := y.Validate(); err != nil {
			return nil, errs.WrapValidation(err, "invalid commitment (2nd operand)")
		}
...
func (*vectorHomomorphicScheme) ScaleCommitment(x *VectorCommitment, n *saferith.Nat) (*VectorCommitment, error) {
	if err := x.Validate(); err != nil {
		return nil, errs.WrapValidation(err, "invalid commitment")
	}
	if n == nil {
...
func (*vectorHomomorphicScheme) CombineOpenings(x *Opening, ys ...*Opening) (*Opening, error) {
	if err := x.Validate(); err != nil {
		return nil, errs.WrapValidation(err, "invalid opening (1st operand)")
	}

	acc := &Opening{
		vector:  make(Vector, len(x.vector)),
	...
	for _, y := range ys {
		...
		if err := y.Validate(); err != nil {
			return nil, errs.WrapValidation(err, "invalid opening (2nd operand)")
		}
...
func (*vectorHomomorphicScheme) ScaleOpening(x *Opening, n *saferith.Nat) (*Opening, error) {
	if err := x.Validate(); err != nil {
		return nil, errs.WrapValidation(err, "invalid opening")
	}
	if n == nil {
		return nil, errs.NewIsNil("scalar is nil")
	}
