...
func TestHappyPathCommitment(t *testing.T) {
	...
	for i, curve := range supportedCurves {
		t.Run(curve.Name(), func(t *testing.T) {
			...
			vector := make([]pedersenveccomm.Message, i+1)
			for j := 0; j <= i; j++ {
...
func TestHappyPathCombine(t *testing.T) {
	...
	for i, curve := range supportedCurves {
		t.Run(curve.Name(), func(t *testing.T) {
			...
			vectorA := make([]pedersenveccomm.Message, i+1)
			vectorB := make([]pedersenveccomm.Message, i+1)
			vectorAplusB := make([]pedersenveccomm.Message, i+1)
			for j := 0; j <= i; j++ {
