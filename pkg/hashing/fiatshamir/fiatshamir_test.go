...
func TestFiatShamirDeterministic(t *testing.T) {
	a := big.NewInt(1)
	hash, err := hashing.HashChain(sha256.New, a.Bytes())
...
func TestFiatShamirEqual(t *testing.T) {
	a := big.NewInt(1)
	pi, err := hashing.HashChain(sha256.New, a.Bytes())
	require.Nil(t, err)
	pi_, err := hashing.HashChain(sha256.New, a.Bytes())
	require.Nil(t, err)
...
func TestFiatShamirNotEqual(t *testing.T) {
	...
	require.Nil(t, err)
	b := big.NewInt(2)
	pi_, err := hashing.HashChain(sha256.New, b.Bytes())
	require.Nil(t, err)
...
func TestFiatShamirOrderDependent(t *testing.T) {
	...
	pi, err := hashing.HashChain(sha256.New, a.Bytes(), b.Bytes())
	require.Nil(t, err)
	pi_, err := hashing.HashChain(sha256.New, a.Bytes(), b.Bytes())
	require.Nil(t, err)
	require.Equal(t, pi, pi_)

	q, _ := hashing.HashChain(sha256.New, b.Bytes(), a.Bytes())
...
func TestFiatShamirExtensionAttackResistance(t *testing.T) {
	...
	pi, err := hashing.HashChain(sha256.New, a.Bytes(), b.Bytes())
	require.Nil(t, err)
	pi_, err := hashing.HashChain(sha256.New, c.Bytes(), d.Bytes())
	require.Nil(t, err)
	require.NotEqual(t, pi, pi_)
	...
	q, err := hashing.HashChain(sha256.New, a.Bytes(), b.Bytes())
	require.Nil(t, err)
	q_, err := hashing.HashChain(sha256.New, c.Bytes(), d.Bytes())
	require.Nil(t, err)
