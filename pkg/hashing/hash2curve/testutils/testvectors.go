...
func SetCurveHasher(curve curves.Curve, tv *TestVector) (hash2curve.CurveHasher, error) {
	appTag, found := strings.CutSuffix(tv.Dst, tv.SuiteName)
	if !found {
		return nil, errs.NewVerificationFailed("could not cut suffix from dst")
	}
	suiteIdSections := strings.Split(tv.SuiteName, hash2curve.DST_ID_SEPARATOR)
	if len(suiteIdSections) != 5 || suiteIdSections[0] != curve.Name() || suiteIdSections[3] != hash2curve.DST_ENC_VAR {
	...
	case strings.Contains(hasherNCurveTag, hash2curve.DST_TAG_SHA256):
		ch = hash2curve.NewCurveHasherSha256(curve, appTag, mapperTag)
	case strings.Contains(hasherNCurveTag, hash2curve.DST_TAG_SHA512):
		ch = hash2curve.NewCurveHasherSha512(curve, appTag, mapperTag)
	case strings.Contains(hasherNCurveTag, hash2curve.DST_TAG_SHAKE256):
...
func NewHash2CurveTestVector(curve curves.Curve) *TestVector {
	switch curve.Name() {
	case constants.P256_NAME:
		return NewHash2Curve_P256_TestVector()
	case constants.ED25519_NAME:
		return NewHash2Curve_Edwards25519_TestVector()
	case constants.K256_NAME:
		return NewHash2Curve_secp256k1_TestVector()
	case constants.CURVE25519_NAME:
		return NewHash2Curve_Curve25519_TestVector()
	case constants.BLS12381G1_NAME:
		return NewHash2Curve_BLS12381G1_TestVector()
	case constants.BLS12381G2_NAME:
		return NewHash2Curve_BLS12381G2_TestVector()
	default:
...
func NewHash2Curve_P256_TestVector() *TestVector {
	return &TestVector{
...
func NewHash2Curve_Curve25519_TestVector() *TestVector {
	return &TestVector{
...
func NewHash2Curve_Edwards25519_TestVector() *TestVector {
	return &TestVector{
...
func NewHash2Curve_secp256k1_TestVector() *TestVector {
	return &TestVector{
...
func NewHash2Curve_BLS12381G1_TestVector() *TestVector {
	return &TestVector{
...
func NewHash2Curve_BLS12381G2_TestVector() *TestVector {
	return &TestVector{
