...
B) Chain the output of the TMMOs to each input block by setting the key:
...
## Use as CS-PRNG
This Hash can be used as a CS-PRNG, and as such it exposes the same interface as
the `nist` package.
...
## PRNG properties
- _SecurityStrength_: The TmmoHash uses AES with either 128 or 256-bit keys, obtaining said level of computational security.
- _SeedSize_: The key size is one of \{16, 32\}. Nonces can have any size, but are recommended to have 16 bytes to match the AES block size.
- _AutomaticReseeding_: NO. This PRNG does not reseed automatically.
- _KeyDerivationFunction_: NO. This PRNG does not use a KDF. 

\[GKWY20\] Efficient and Secure Multiparty Computation from Fixed-Key Block Ciphers. C. Guo, J Katz, X. Wang. 2019. https://eprint.iacr.org/2019/074.pdf    
[1]: "https://eprint.iacr.org/2019/074.pdf"
