...
import (
	...
	"github.com/bronlabs/bron-crypto/pkg/mpc/meta/signatures/ecdsa/dkls23/signing_softspoken"
...
func TestRunner_HappyPath(t *testing.T) {
	...
	for id := range signingQuorum.Iter() {
		shard, ok := shards[id]
		require.True(t, ok)
		runner, err := signing_softspoken.NewRunner(ctxs[id], suite, ntu.CBORRoundTrip(t, shard), message, pcg.NewRandomised())
		require.NoError(t, err)
		runners[id] = runner
	}
