...
func Test_PreSignatureHappyPath(t *testing.T) {
	t.Parallel()
			...
			for hashFuncName, hashFunc := range TestHashFuncs {
				t.Run(hashFuncName, func(t *testing.T) {
					...
					t.Run("secp256k1", func(t *testing.T) {
						...
						testPreSignatureHappyPath(t, suite, as)
					})
...
func testPreSignatureHappyPath[P curves.Point[P, B, S], B algebra.PrimeFieldElement[B], S algebra.PrimeFieldElement[S]](t *testing.T, suite *ecdsa.Suite[P, B, S], accessStructure accessstructures.Monotone) {
	t.Helper()
	...
	for ids := range sliceutils.KCoveringCombinations(accessStructure.Shareholders().List(), 1) {
		...
		preSignatures := make(map[sharing.ID]*dkls23.PreSignature[P, B, S])
		for id, c := range cosigners {
			preSignatures[id], err = c.PreSign(r5bIn[id], r5uIn[id])
			require.NoError(t, err)
			preSignatures[id] = ntu.CBORRoundTrip(t, preSignatures[id])
		}

		// the message is only chosen after presigning
