...
const (
	// ProtocolName identifies Lindell17 signing in runner notifications.
	ProtocolName = "Lindell17_Signing"
...
// NewPrimaryRunner constructs the primary party's network runner. The session
// must be an authorized two-party quorum, niCompiler must select Fischlin or
// Randomised Fischlin, and prng must be cryptographically secure. Both parties
// must use the same niCompiler.
...
// NewSecondaryRunner constructs the secondary party's network runner. The
// session must be an authorized two-party quorum, niCompiler must select
// Fischlin or Randomised Fischlin, and prng must be cryptographically secure.
// Both parties must use the same niCompiler.
...
func (r *primaryRunner[P, B, S]) Run(ctx context.Context, rt *network.Router, notificationCallback network.NotificationCallback) (*ecdsa.Signature[S], error) {
	...
	err = exchange.UnicastSend(ctx, rt, r1CorrelationID, r1u.Freeze())
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("cannot send round 1")
	}
	network.NotifyRoundCompleted(notificationCallback, ProtocolName, 1)

	r2In, err := exchange.UnicastReceive[*Round2OutputP2P[P, B, S]](ctx, rt, r2CorrelationID, hashset.NewComparable(r.cosigner.secondarySharingID).Freeze())
	if err != nil {
	...
	err = exchange.UnicastSend(ctx, rt, r3CorrelationID, r3u.Freeze())
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("cannot send round 3")
	}
	network.NotifyRoundCompleted(notificationCallback, ProtocolName, 3)

	r4In, err := exchange.UnicastReceive[*Round4OutputP2P[P, B, S]](ctx, rt, r4CorrelationID, hashset.NewComparable(r.cosigner.secondarySharingID).Freeze())
	if err != nil {
...
func (r *secondaryRunner[P, B, S]) Run(ctx context.Context, rt *network.Router, notificationCallback network.NotificationCallback) (*ecdsa.Signature[S], error) {
	r1In, err := exchange.UnicastReceive[*Round1OutputP2P[P, B, S]](ctx, rt, r1CorrelationID, hashset.NewComparable(r.cosigner.primarySharingID).Freeze())
	if err != nil {
	...
	err = exchange.UnicastSend(ctx, rt, r2CorrelationID, r2u.Freeze())
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("cannot send round 2")
	}
	network.NotifyRoundCompleted(notificationCallback, ProtocolName, 2)

	r3In, err := exchange.UnicastReceive[*Round3OutputP2P[P, B, S]](ctx, rt, r3CorrelationID, hashset.NewComparable(r.cosigner.primarySharingID).Freeze())
	if err != nil {
	...
	err = exchange.UnicastSend(ctx, rt, r4CorrelationID, r4u.Freeze())
	if err != nil {
