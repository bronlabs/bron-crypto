...
// DecomposeTwoThirds splits a scalar into two outputs such that the scalar is
// three times the first plus the second, with both outputs in [q/3, 2q/3).
...
func DecomposeTwoThirds[S algebra.PrimeFieldElement[S]](scalar S, prng io.Reader) (xPrime, xDoublePrime S, err error) {
	...
	if utils.IsNil(scalar) {
		return nilS, nilS, ErrInvalidArgument.WithMessage("scalar is nil")
	}
	if prng == nil {
		return nilS, nilS, ErrInvalidArgument.WithMessage("prng is nil")
	}
	...
	if err != nil {
		return nilS, nilS, errs.Wrap(err).WithMessage("could not classify first scalar interval")
	}
	inSecond, err := inRange(3, 6, scalar)
	if err != nil {
		return nilS, nilS, errs.Wrap(err).WithMessage("could not classify second scalar interval")
	}
	inThird, err := inRange(6, 9, scalar)
	if err != nil {
		return nilS, nilS, errs.Wrap(err).WithMessage("could not classify third scalar interval")
	}
	inFourth, err := inRange(9, 12, scalar)
	if err != nil {
		return nilS, nilS, errs.Wrap(err).WithMessage("could not classify fourth scalar interval")
	}
	inFifth, err := inRange(12, 15, scalar)
	if err != nil {
		return nilS, nilS, errs.Wrap(err).WithMessage("could not classify fifth scalar interval")
	}
	inSixth, err := inRange(15, 18, scalar)
	if err != nil {
		return nilS, nilS, errs.Wrap(err).WithMessage("could not classify sixth scalar interval")
	}
...
func randomInEighteenth[S algebra.PrimeFieldElement[S]](lowBoundInclusive, highBoundExclusive uint64, field algebra.PrimeField[S], prng io.Reader) (S, error) {
	...
	h, _, err := h18.Add(num.Z().FromUint64(17)).EuclideanDivVarTime(num.Z().FromUint64(18))
	if err != nil {
