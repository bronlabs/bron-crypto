...
import (
	...
	"github.com/bronlabs/bron-crypto/pkg/mpc/signatures/ecdsa/cggmp24"
	"github.com/bronlabs/bron-crypto/pkg/signatures/ecdsa"
)

// DealShards deals CGGMP24 shards for the given ECDSA curve and access structure.
func DealShards[P curves.Point[P, B, S], B algebra.PrimeFieldElement[B], S algebra.PrimeFieldElement[S]](curve ecdsa.Curve[P, B, S], prng io.Reader, as accessstructures.Monotone) (map[sharing.ID]*cggmp24.Shard[P, B, S], error) {
	return dealShards(curve, prng, as, base.IFCKeyLength)
}

// DealShardsWithKeyLen used only for testing.
func DealShardsWithKeyLen[P curves.Point[P, B, S], B algebra.PrimeFieldElement[B], S algebra.PrimeFieldElement[S]](tb testing.TB, curve ecdsa.Curve[P, B, S], prng io.Reader, as accessstructures.Monotone, keyLen int) map[sharing.ID]*cggmp24.Shard[P, B, S] {
	tb.Helper()
...
func dealShards[P curves.Point[P, B, S], B algebra.PrimeFieldElement[B], S algebra.PrimeFieldElement[S]](curve ecdsa.Curve[P, B, S], prng io.Reader, as accessstructures.Monotone, keyLen int) (map[sharing.ID]*cggmp24.Shard[P, B, S], error) {
	...
	shards := make(map[sharing.ID]*cggmp24.Shard[P, B, S])
	for id := range as.Shareholders().Iter() {
		auxInfo := &cggmp24.AuxInfo{
			PaillierSecretKey:      paillierSecretKeys[id],
			PaillierPublicKeys:     paillierPublicKeys,
			RingPedersenSecretKey:  ringPedersenSecretKeys[id],
			RingPedersenPublicKeys: ringPedersenPublicKeys,
		}
		baseShard, _ := baseShards.Get(id)
		shard, err := cggmp24.NewShard(baseShard, auxInfo)
		if err != nil {
