...
func NewMislayer[G algebra.PrimeGroupElement[G, S], S algebra.PrimeFieldElement[S]](ctx *session.Context, as *threshold.Threshold, group algebra.PrimeGroup[G, S]) (*Mislayer[G, S], error) {
	...
	recovererQuorum := make([]sharing.ID, 0, ctx.Quorum().Size()-1)
	for _, id := range ctx.Quorum().List() {
		if id != ctx.HolderID() {
			recovererQuorum = append(recovererQuorum, id)
		}
	}
	if !as.IsQualified(recovererQuorum...) {
