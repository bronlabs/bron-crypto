...
func (r *Recoverer[G, S]) Round2(r1b network.RoundMessages[*Round1Broadcast[G, S]], r1u network.RoundMessages[*Round1P2P[G, S]]) (network.OutgoingUnicasts[*Round2P2P[G, S]], error) {
	blindedShare := r.state.blindShare.Add(r.shard.Share())
	for id := range r.recoverersCtx.OtherPartiesOrdered() {
		u, ok := r1u.Get(id)
		if !ok {
			return nil, ErrFailed.WithMessage("missing share")
		}
		b, ok := r1b.Get(id)
		if !ok {
			return nil, ErrFailed.WithMessage("missing verification vector")
		}
		...
		if !verificationVector.Eval(r.field.FromUint64(uint64(r.mislayerID))).IsOpIdentity() {
			return nil, base.ErrAbort.WithTag(base.IdentifiableAbortPartyIDTag, id).WithMessage("invalid share")
		}

		share := u.BlindShare
		err := r.scheme.Verify(share, verificationVector)
		if err != nil {
			return nil, errs.Wrap(err).WithTag(base.IdentifiableAbortPartyIDTag, id).WithMessage("cannot verify share")
		}
...
func (m *Mislayer[G, S]) Round3(r2u network.RoundMessages[*Round2P2P[G, S]]) (output *Output[G, S], err error) {
	xs := []S{}
	ys := []S{}

	var verificationVector feldman.VerificationVector[G, S]
	for id := range m.ctx.OtherPartiesOrdered() {
		u, ok := r2u.Get(id)
		if !ok {
			return nil, ErrFailed.WithMessage("missing message")
		}
		if verificationVector == nil {
			verificationVector = u.VerificationVector
		} else if !verificationVector.Equal(u.VerificationVector) {
			return nil, base.ErrAbort.WithMessage("mislayer verification vector does not match")
		}
