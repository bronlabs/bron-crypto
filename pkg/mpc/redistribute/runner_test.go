...
func TestRunner_HappyPathRecover_NoTrustedAnchor(t *testing.T) {
	t.Parallel()

	testRunnerHappyRecoverNoTrustedAnchor(t, k256.NewCurve())
}
...
func testRunnerHappyRecover[G algebra.PrimeGroupElement[G, S], S algebra.PrimeFieldElement[S]](tb testing.TB, group algebra.PrimeGroup[G, S]) {
	...
	for id, ctx := range ctxs {
		...
		runner, err := redistribute.NewRunner(ctx, recoverers, shard, as, pcg.NewRandomised(), redistribute.WithTrustedAnchorID(2))
		require.NoError(tb, err)
		runners[id] = runner
	}
...
func testRunnerHappyRecoverNoTrustedAnchor[G algebra.PrimeGroupElement[G, S], S algebra.PrimeFieldElement[S]](tb testing.TB, group algebra.PrimeGroup[G, S]) {
	tb.Helper()
