...
func TestRunner_HappyPath(t *testing.T) {
	...
	message := []byte("hello from dkls23 sign_softspoken runner")
	...
	for id := range quorum.Iter() {
		...
		require.True(t, ok)
		tapes[id] = tape.Clone()
		runner, err := sign_softspoken.NewRunner(sessionID, quorum, suite, shard, message, pcg.NewRandomised(), tapes[id])
		require.NoError(t, err)
		runners[id] = runner
	}
