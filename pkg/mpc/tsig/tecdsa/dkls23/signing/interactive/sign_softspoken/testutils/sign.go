...
func RunDKLs23SignSoftspokenOT[P curves.Point[P, B, S], B algebra.PrimeFieldElement[B], S algebra.PrimeFieldElement[S]](tb testing.TB, shards map[sharing.ID]*dkls23.Shard[P, B, S], quorum ds.Set[sharing.ID], message []byte, hashFunc func() hash.Hash) *ecdsa.Signature[S] {
	...
	cosignersMap := make(map[sharing.ID]*sign_softspoken.Cosigner[P, B, S])
	for id := range quorum.Iter() {
		shard, ok := shards[id]
		require.True(tb, ok)
		cosignersMap[id], err = sign_softspoken.NewCosigner(ctxs[id], ecdsaSuite, ntu.CBORRoundTrip(tb, shard), prng)
		require.NoError(tb, err)
	}
	...
	for _, cosigner := range cosigners {
		ps, err := cosigner.Round3(r3bi[cosigner.SharingID()], r3ui[cosigner.SharingID()], message)
		require.NoError(tb, err)
