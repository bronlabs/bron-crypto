...
func newAdditiveScheme[E additive.GroupElement[E]](g additive.Group[E], shareholders ds.Set[sharing.ID]) (*additive.Scheme[E], error) {
	ac, err := accessstructures.NewUnanimityAccessStructure(shareholders)
	if err != nil {
		return nil, err
	}
...
func shareValidationCases[E additive.GroupElement[E]](t *testing.T, scheme *additive.Scheme[E], field interface {
	...
	tests := []struct {
		name         string
		shareFunc    func(ac *accessstructures.Unanimity) (*additive.Share[E], error)
		accessStruct *accessstructures.Unanimity
		expectError  bool
		errorIs      error
	}{
		{
...
func TestSumToSecret_Randomness(t *testing.T) {
	...
	t.Run("different outputs for different prng states", func(t *testing.T) {
		...
		shares1, err := additive.SumToSecret(secret.Value(), group.Random, pcg.NewRandomised(), 5)
		require.NoError(t, err)

		shares2, err := additive.SumToSecret(secret.Value(), group.Random, pcg.NewRandomised(), 5)
		require.NoError(t, err)
	...
	t.Run("deterministic with same prng seed", func(t *testing.T) {
		...
		shares1, err := additive.SumToSecret(secret.Value(), group.Random, pcg.New(seed1, seed2), 5)
		require.NoError(t, err)

		shares2, err := additive.SumToSecret(secret.Value(), group.Random, pcg.New(seed1, seed2), 5)
		require.NoError(t, err)
...
func TestSumToSecret_ErrorCases(t *testing.T) {
	...
	t.Run("short prng", func(t *testing.T) {
		...
		shares, err := additive.SumToSecret(secret.Value(), group.Random, bytes.NewReader([]byte{1}), 3)
		require.Error(t, err)
		require.Nil(t, shares)
	})
}
...
func TestSumToSecret_Properties(t *testing.T) {
	...
	t.Run("varying l produces valid sharings", func(t *testing.T) {
		...
		for l := 1; l <= 20; l++ {
			shares, err := additive.SumToSecret(secret.Value(), group.Random, pcg.NewRandomised(), l)
			require.NoError(t, err)
...
func TestSumToSecret_AdditiveHomomorphism(t *testing.T) {
	...
	t.Run("sum of sharings equals sharing of sum", func(t *testing.T) {
		...
		shares1, err := additive.SumToSecret(secret1.Value(), group.Random, pcg.New(seed1, seed2), 5)
		require.NoError(t, err)

		shares2, err := additive.SumToSecret(secret2.Value(), group.Random, pcg.New(seed1, seed2), 5)
		require.NoError(t, err)
