...
func TestToAdditiveEdgeCases(t *testing.T) {
	...
	t.Run("single shareholder qualified set", func(t *testing.T) {
		...
		_, err := unanimity.NewUnanimityAccessStructure(
			singleID.Freeze(),
		)
		require.Error(t, err)
...
func BenchmarkReconstruct(b *testing.B) {
	...
	for _, config := range benchConfigs {
		b.Run(config.name, func(b *testing.B) {
			...
			// Pick exactly threshold shares for reconstruction.
...
func TestArbitraryShareholderIDs(t *testing.T) {
	...
	qualifiedSet, err := unanimity.NewUnanimityAccessStructure(shareholders)
	require.NoError(t, err)
