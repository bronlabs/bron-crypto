...
func NewDealerFunc[FE algebra.PrimeFieldElement[FE]](randomColumn *mat.Matrix[FE], msp *msp.MSP[FE]) (*DealerFunc[FE], error) {
	if msp == nil {
...
// column count.
func NewLiftedDealerFunc[E algebra.PrimeGroupElement[E, FE], FE algebra.PrimeFieldElement[FE]](verificationVector *mat.ModuleValuedMatrix[E, FE], msp *msp.MSP[FE]) (*LiftedDealerFunc[E, FE], error) {
	...
	liftedLambda, err := mat.LeftAction(msp.Matrix(), verificationVector)
	if err != nil {
...
type LiftedDealerFunc[E algebra.PrimeGroupElement[E, FE], FE algebra.PrimeFieldElement[FE]] struct {
	verificationVector *mat.ModuleValuedMatrix[E, FE]
	msp                *msp.MSP[FE]
...
func (d *LiftedDealerFunc[E, FE]) LiftedSecret() E {
	liftedSecretMat, err := mat.LeftAction(d.msp.TargetVector(), d.verificationVector)
	if err != nil {
		panic(errs.Wrap(err).WithMessage("failed to compute lifted secret = target * lifted_r"))
	}
	out, err := liftedSecretMat.Get(0, 0)
	if err != nil {
...
func (d *LiftedDealerFunc[E, FE]) VerificationVector() *mat.ModuleValuedMatrix[E, FE] {
