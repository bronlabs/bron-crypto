...
func GenerateOTinputs(Xi, L int) (
	...
	if L < 1 || Xi < 1 || Xi%8 != 0 {
		return nil, nil, errs.NewInvalidLength(" cannot generate random inputs for L=%d, Xi=%d", L, Xi)
	}
	...
	for i := 0; i < Xi; i++ {
		...
		for l := 0; l < L; l++ {
			...
			if err0 != nil || err1 != nil {
				return nil, nil, errs.WrapRandomSampleFailed(err, "could not generate random message")
			}
		}
	}
...
func ValidateOT(
	...
	if len(receiverChoiceBits) != Xi/8 || len(receiverChosenMessages) != Xi || len(senderMessages) != Xi {
		return errs.NewInvalidLength("ROT output length mismatch (should be %d, is: %d)",
	...
	for i := 0; i < Xi; i++ {
		if len(receiverChosenMessages[i]) != L || len(senderMessages[i][0]) != L || len(senderMessages[i][1]) != L {
			return errs.NewInvalidLength("ROT output message length mismatch (should be %d, is: %d, %d, %d)",
		...
		if !bytes.Equal(receiverChosenMessages[i][0][:], senderMessages[i][choice][0][:]) {
			return errs.NewVerificationFailed("ROT output mismatch for index %d", i)
		}
	}
	return nil
}
