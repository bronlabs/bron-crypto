...
func (s *Sender) Round1() (r1out *Round1P2P, err error) {
	...
	// step 1.1 (KA.R)
	s.MyEsk, err = s.Curve().ScalarField().Random(s.Prng())
	if err != nil {
...
func (r *Receiver) Round2(r1out *Round1P2P) (r2out *Round2P2P, err error) {
	...
	for i := 0; i < r.Xi; i++ {
		...
		for l := 0; l < r.L; l++ {
			// step 2.2 (KA.R)
			b_i, err := r.Curve().ScalarField().Random(r.Prng())
			if err != nil {
			...
			sc, err := r.Curve().ScalarField().Random(r.Prng())
			if err != nil {
	...
	for i := 0; i < s.Xi; i++ {
		...
		for l := 0; l < s.L; l++ {
			for j := byte(0); j < 2; j++ {
				...
				P := s.Curve().ScalarBaseMult(sc).ClearCofactor().Add(r2out.Phi[i][j][l])
				// step 3.3 (KA.key_1)
