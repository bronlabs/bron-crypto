...
func NewSender(curve *curves.Curve, batchSize int, uniqueSessionId [DigestSize]byte, transcript *merlin.Transcript) (*Sender, error) {
...
func NewReceiver(curve *curves.Curve, batchSize int, uniqueSessionId [DigestSize]byte, transcript *merlin.Transcript) (*Receiver, error) {
...
func (sender *Sender) Round1ComputeAndZkpToPublicKey() (*schnorr.Proof, error) {
	...
	prover, err := schnorr.NewProver(sender.curve.NewGeneratorPoint(), uniqueSessionId[:], sender.transcript)
	if err != nil {
...
func (receiver *Receiver) Round2VerifySchnorrAndPadTransfer(proof *schnorr.Proof) ([]ReceiversMaskedChoices, error) {
	...
	if err := schnorr.Verify(receiver.curve.NewGeneratorPoint(), proof, uniqueSessionId[:], receiver.transcript); err != nil {
	...
	for i := 0; i < receiver.batchSize; i++ {
		...
		output, err := hashing.Hash(sha3.New256, uniqueSessionId[:], []byte{byte(i)}, rho.ToAffineCompressed())
		if err != nil {
...
func (sender *Sender) Round3PadTransfer(compressedReceiversMaskedChoice []ReceiversMaskedChoices) ([]OtChallenge, error) {
	...
	for i := 0; i < sender.batchSize; i++ {
		...
		for k := 0; k < keyCount; k++ {
			...
			output, err := hashing.Hash(sha3.New256, uniqueSessionId[:], []byte{byte(i)}, baseEncryptionKeyMaterial[k].ToAffineCompressed())
			if err != nil {
