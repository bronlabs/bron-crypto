...
func RunVSOT(batchSize, messageLength int, curve curves.Curve, uniqueSessionId []byte, prng io.Reader) (*ot.SenderRotOutput, *ot.ReceiverRotOutput, error) {
	receiver, err := vsot.NewReceiver(batchSize, messageLength, curve, uniqueSessionId, randomisedFischlin.Name, nil, prng)
	if err != nil {
		return nil, nil, errs.WrapFailed(err, "constructing OT receiver in run VSOT")
	}
	sender, err := vsot.NewSender(batchSize, messageLength, curve, uniqueSessionId, randomisedFischlin.Name, nil, prng)
	if err != nil {
