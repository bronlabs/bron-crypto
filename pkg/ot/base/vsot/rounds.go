...
func (r *Receiver) Round2(r1out *Round1P2P) (r2out *Round2P2P, err error) {
	...
	for i := 0; i < r.Protocol.Xi; i++ {
		...
		for l := 0; l < r.Protocol.L; l++ {
			...
			subtle.ConstantTimeCopy(int(r.Output.Choices.Get(i)), r2out.MaskedChoices[i][l], option1Bytes)
...
func (r *Receiver) Round4(r3out *Round3P2P) (*Round4P2P, error) {
	...
	for i := 0; i < r.Protocol.Xi; i++ {
		...
		for l := 0; l < r.Protocol.L; l++ {
			...
			subtle.ConstantTimeCopy(int(r.Output.Choices.Get(i)), r4out.Responses[i][l][:], alternativeChallengeResponse[:])
		}
	}
...
func (r *Receiver) Round6(r5out *Round5P2P) error {
	...
	for i := 0; i < r.Protocol.Xi; i++ {
		for l := 0; l < r.Protocol.L; l++ {
			...
			choice := int(r.Output.Choices.Get(i))
