...
func (s *Sender[G, S]) Round1() (r1out *Round1P2P[G, S], err error) {
	...
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("generating a")
	}

	// step 1.2 (KA.msg_1)
	ms, err := s.ka.Msg1(s.state.a)
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("creating msg1")
	}
...
func (r *Receiver[G, S]) Round2(r1out *Round1P2P[G, S], choices []byte) (r2out *Round2P2P[G, S], receiverOut *ReceiverOutput[S], err error) {
	...
	for i := range r.suite.Xi() {
		...
		for l := range r.suite.L() {
			...
			if err != nil {
				return nil, nil, errs.Wrap(err).WithMessage("generating random scalar bi")
			}

			// step 2.3 (KA.msg_2)
			mRi, err := r.ka.Msg2(bi, r1out.Ms)
			if err != nil {
				return nil, nil, errs.Wrap(err).WithMessage("creating msg2")
			}
			...
			if err != nil {
				return nil, nil, errs.Wrap(err).WithMessage("computing shared bytes for KA.key_2")
			}

			// step 2.5,2.6 (POPF.Program)
			phi[i][0][l], phi[i][1][l], err = f.Program(ci, mRi, r.prng)
			if err != nil {
				return nil, nil, errs.Wrap(err).WithMessage("generating random scalar sc")
			}
		}
	}
...
func (s *Sender[G, S]) Round3(r2out *Round2P2P[G, S]) (senderOut *SenderOutput[S], err error) {
	...
	if err != nil {
		return nil, errs.Wrap(err).WithMessage("creating popf")
	}
	...
	for i := range s.suite.Xi() {
		for l := range s.suite.L() {
			for j := range byte(2) {
				...
				if err != nil {
					return nil, errs.Wrap(err).WithMessage("popf eval")
				}
				...
				if err != nil {
					return nil, errs.Wrap(err).WithMessage("computing shared bytes for KA.key_2")
				}
			}
		}
	}
