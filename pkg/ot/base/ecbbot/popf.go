...
func (f *Popf[GE, SE]) Program(x byte, y GE, prng io.Reader) (s0, s1 GE, err error) {
	...
	if x == 0 {
		...
		if err != nil {
			return nilGE, nilGE, errs.Wrap(err).WithMessage("cannot sample point")
		}
		h0, err := f.h0(s1)
		if err != nil {
			return nilGE, nilGE, errs.Wrap(err).WithMessage("s1")
		}
		s0 = y.Op(h0.OpInv())
	} else {
...
func (f *Popf[GE, SE]) Eval(s0, s1 GE, x byte) (y GE, err error) {
	...
	if x == 0 {
		h0, err := f.h0(s1)
		if err != nil {
			return nilGE, errs.Wrap(err).WithMessage("s1")
		}
		y = s0.Op(h0)
	} else {
		h1, err := f.h1(s0)
...
func (f *Popf[GE, SE]) h0(p GE) (GE, error) {
	...
	if err != nil {
		return nilGE, errs.Wrap(err).WithMessage("hash to curve failed")
	}
	return p0, nil
}
