...
func (f *Popf[GE, SE]) Program(x byte, y GE, prng io.Reader) (s0, s1 GE, err error) {
	var nilGE GE
	if x > 1 || prng == nil {
		return nilGE, nilGE, errs.NewValidation("invalid arguments")
	}

	if x == 0 {
		s1, err = f.group.Random(prng)
		if err != nil {
			return nilGE, nilGE, errs.WrapRandomSample(err, "cannot sample point")
		}
		h0, err := f.h0(s1)
		if err != nil {
			return nilGE, nilGE, errs.WrapHashing(err, "s1")
		}
		s0 = y.Op(h0.OpInv())
	} else {
...
func (f *Popf[GE, SE]) Eval(s0, s1 GE, x byte) (y GE, err error) {
	var nilGE GE
	if x > 1 {
		return nilGE, errs.NewValidation("invalid arguments")
	}

	if x == 0 {
		h0, err := f.h0(s1)
		if err != nil {
			return nilGE, errs.WrapHashing(err, "s1")
		}
		y = s0.Op(h0)
	} else {
		h1, err := f.h1(s0)
...
func (f *Popf[GE, SE]) h0(p GE) (GE, error) {
	...
	if err != nil {
		return nilGE, errs.WrapFailed(err, "hash to curve failed")
	}
	return p0, nil
}
