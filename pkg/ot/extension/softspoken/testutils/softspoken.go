...
func RunSoftspokenOTe(
	...
	// Setup OTe
	sender, err := softspoken.NewSoftspokenSender(baseOtReceiverOutput, sid, nil, curve, rand, nil, L, Xi)
	if err != nil {
		return nil, nil, errs.WrapFailed(err, "could not create softspoken sender")
	}
	receiver, err := softspoken.NewSoftspokenReceiver(baseOtSenderOutput, sid, nil, curve, rand, nil, L, Xi)
	if err != nil {
