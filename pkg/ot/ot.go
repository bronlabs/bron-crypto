...
func NewParticipant(Xi, L int, curve curves.Curve, sid []byte, label string, t transcripts.Transcript, csprng io.Reader) (*Participant, error) {
	if Xi&0x07 != 0 || Xi < 1 { // `Enforce batchSize % 8 != 0`
		return nil, errs.NewInvalidArgument("batch size should be a positive multiple of 8")
	}
	if L < 1 {
		return nil, errs.NewInvalidArgument("message length should be positive")
	}
	if csprng == nil {
...
func (sROT *SenderRotOutput) Round2Encrypt(OTmessagePairs []MessagePair) (masks []OneTimePadedMaskPair, err error) {
	...
	if len(OTmessagePairs) != Xi {
		return nil, errs.NewInvalidArgument("number of OT message pairs should be Xi (%d != %d)", len(OTmessagePairs), Xi)
	}
	masks = make([]OneTimePadedMaskPair, Xi)
	for j := 0; j < Xi; j++ {
		if len(OTmessagePairs[j][0]) != L || len(OTmessagePairs[j][1]) != L {
			return nil, errs.NewInvalidArgument("OT message[%d] length should be L (%d != %d || %d != %d )",
...
func (rROT *ReceiverRotOutput) Round3Decrypt(masks []OneTimePadedMaskPair) (OTchosenMessages []ChosenMessage, err error) {
	...
	if len(masks) != Xi {
		return nil, errs.NewInvalidArgument("number of masks should be Xi (%d != %d)", len(masks), Xi)
	}
	OTchosenMessages = make([]ChosenMessage, Xi)
	for j := 0; j < Xi; j++ {
		if len(masks[j][0]) != L || len(masks[j][1]) != L {
			return nil, errs.NewInvalidArgument("mask[%d] length should be L (%d != %d)", j, len(masks[j][0]), L)
		}
...
func (sROT *SenderRotOutput) Round2CreateCorrelation(a []CorrelatedMessage) (z_A []CorrelatedMessage, tau []CorrelationMask, err error) {
	...
	L := len(sROT.Messages[0][0])
	if len(a) != Xi {
		return nil, nil, errs.NewInvalidArgument("senderInput size should be same as batch size (%d != %d)", len(a), Xi)
	}
	...
	for j := 0; j < Xi; j++ {
		if len(a[j]) != L {
			return nil, nil, errs.NewInvalidArgument("a[%d] length should be L (%d != %d)", j, len(a[j]), L)
		}
		...
		for l := 0; l < L; l++ {
			...
			if z_A[j][l], err = scalarField.Hash(sROT.Messages[j][0][l][:]); err != nil {
				return nil, nil, errs.WrapHashingFailed(err, "bad hashing s_0_j to scalar for ROT->COT")
			}
			// τ_j = ECS(s_1_j) - z_A_j + α_j
			if tau[j][l], err = scalarField.Hash(sROT.Messages[j][1][l][:]); err != nil {
				return nil, nil, errs.WrapHashingFailed(err, "bad hashing s_1_j to scalar for ROT->COT")
			}
...
func (rROT *ReceiverRotOutput) Round3ApplyCorrelation(tau []CorrelationMask) (z_B []CorrelatedMessage, err error) {
	...
	L := len(rROT.ChosenMessages[0])
	if len(tau) != Xi {
		return nil, errs.NewInvalidArgument("length of tau should be same as batch size (%d != %d)",
			len(tau), Xi)
	}
	...
	for j := 0; j < Xi; j++ {
		if len(tau[j]) != L {
			return nil, errs.NewInvalidArgument("tau[%d] length should be L (%d != %d)", j, len(tau[j]), L)
		}
		z_B[j] = make(CorrelatedMessage, L)
		for l := 0; l < L; l++ {
			r_x, err := scalarField.Hash(rROT.ChosenMessages[j][l][:])
			if err != nil {
				return nil, errs.WrapHashingFailed(err, "bad hashing r_x_j to scalar for ROT -> COT")
			}
			r_x = r_x.Neg()
			bit := rROT.Choices.Select(j)
			// z_B_j = τ_j - ECS(r_x_j)  if x_j == 1
			//       =     - ECS(r_x_j)  if x_j == 0
			z_B[j][l] = ct.ConstantTimeSelectScalar(bit, tau[j][l].Add(r_x), r_x)
		}
	}
	return z_B, nil
}
