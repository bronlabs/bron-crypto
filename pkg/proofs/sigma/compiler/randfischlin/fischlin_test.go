...
func TestRandFischlin_NilCompilerInput(t *testing.T) {
	...
	// nil protocol
	_, err = randfischlin.NewCompiler[
	...
	// nil prng
	_, err = randfischlin.NewCompiler(schnorrProtocol, nil)
	require.Error(t, err)
}

func TestRandFischlin_TranscriptsMatch(t *testing.T) {
	t.Parallel()

	t.Run("k256", func(t *testing.T) {
		t.Parallel()
		testRandFischlinTranscriptsMatch(t, k256.NewCurve())
	})
	t.Run("p256", func(t *testing.T) {
		t.Parallel()
		testRandFischlinTranscriptsMatch(t, p256.NewCurve())
	})
}
