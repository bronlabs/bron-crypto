...
func (p prover[X, W, A, S, Z]) Prove(statement X, witness W) (compiler.NIZKPoKProof, error) {
	p.transcript.AppendBytes(statementLabel, p.sigmaProtocol.SerializeStatement(statement))
	...
	p.transcript.AppendBytes(commitmentLabel, p.sigmaProtocol.SerializeCommitment(a))
