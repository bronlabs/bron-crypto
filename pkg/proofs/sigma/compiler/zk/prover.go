...
type Prover[X sigma.Statement, W sigma.Witness, A sigma.Commitment, S sigma.State, Z sigma.Response] struct {
	...
	challengeCommitment pedersen_comm.Commitment
	witness             W
	state               S
}
...
func (p *Prover[X, W, A, S, Z]) Round2(eCommitment pedersen_comm.Commitment) (A, error) {
	var zero A
	if p.round != 2 {
		return zero, errs.NewRound("r != 2 (%d)", p.round)
	}

	p.tape.AppendPoints(challengeCommitmentLabel, eCommitment)
...
func (p *Prover[X, W, A, S, Z]) Round4(challengeOpening *commitments.Opening[pedersen_comm.Message, pedersen_comm.Witness]) (Z, error) {
	var zero Z
