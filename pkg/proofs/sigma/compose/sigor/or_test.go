...
import (
	...
	sigmaCompose "github.com/bronlabs/krypton-primitives/pkg/proofs/sigma/compose/sigor"
	"github.com/bronlabs/krypton-primitives/pkg/transcripts/hagrid"
)

func Test_SchnorrOrSchnorr(t *testing.T) {
	t.Parallel()
	...
	for i, c := range supportedCurves {
		...
		t.Run(curve.Name(), func(t *testing.T) {
			...
			t.Run("first true, second false", func(t *testing.T) {
				...
				prover, err := sigma.NewProver(sessionId, proverTranscript, orProtocol, statement, witness)
				require.NoError(t, err)
				verifier, err := sigma.NewVerifier(sessionId, verifierTranscript, orProtocol, statement, prng)
				require.NoError(t, err)
...
func Test_SchnorrOrSchnorrSimulator(t *testing.T) {
	t.Parallel()
	...
	for _, c := range supportedCurves {
		...
		t.Run(curve.Name(), func(t *testing.T) {
			...
			accepted := orProtocol.Verify(statement, commitment, challenge, response)
			require.NoError(t, accepted)
		})
	}
}
