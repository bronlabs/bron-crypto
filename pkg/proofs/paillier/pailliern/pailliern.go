...
var P = (*numct.Nat)(new(saferith.Nat).SetBig(pBig, pBig.BitLen()))
...
func (p *Prover) Prove() (proof *Proof, statement *paillier.PublicKey, err error) {
	rhos, err := extractRhos(p.tape, p.PrivateKey().Arithmetic().CrtModN.N)
	if err != nil {
		return nil, nil, errs.WrapFailed(err, "cannot create a proof")
	}

	var nInv *numct.Nat
	if ok := p.PrivateKey().Arithmetic().CrtModN.ModInv(nInv, p.PrivateKey().Arithmetic().CrtModN.N.Nat()); ok == ct.False {
		return nil, nil, errs.NewFailed("cannot invert N")
	}
	sigmas := make([]*numct.Nat, M)
	p.PrivateKey().Arithmetic().CrtModN.MultiBaseExp(sigmas, rhos, nInv)
	proof = &Proof{
		Sigmas: sigmas,
	}
