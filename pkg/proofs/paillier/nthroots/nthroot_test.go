...
func Test_HappyPathInteractive(t *testing.T) {
	...
	bigN, err := saferith_ex.NewTwoPrimePowersModulus(p, 1, q, 1)
	require.NoError(t, err)
	bigNSquared, err := saferith_ex.NewTwoPrimePowersModulus(p, 2, q, 2)
	require.NoError(t, err)

	yInt, err := crand.Int(prng, bigN.Nat().Big())
	require.NoError(t, err)
	y := new(saferith.Nat).SetBig(yInt, 256)
	x := new(saferith.Nat).Exp(y, bigN.Nat(), bigNSquared.Modulus())
...
func Test_InvalidRootInteractive(t *testing.T) {
	...
	y1Int, err := crand.Int(prng, bigN.Nat().Big())
	require.NoError(t, err)
	y1 := new(saferith.Nat).SetBig(y1Int, 256)
	x1 := new(saferith.Nat).Exp(y1, bigN.Nat(), bigNSquared.Modulus())

	y2Int, err := crand.Int(prng, bigN.Nat().Big())
	require.NoError(t, err)
	y2 := new(saferith.Nat).SetBig(y2Int, 256)
	x2 := new(saferith.Nat).Exp(y2, bigN.Nat(), bigNSquared.Modulus())
...
func doInteractiveProof(x, y *saferith.Nat, bigN, bigNN saferith_ex.Modulus, prng io.Reader) (err error) {
	sessionId := []byte("nthRootSession")
	appLabel := "NthRoot"
