...
func TestHappyPathCommitment(t *testing.T) {
	...
	for i, curve := range supportedCurves {
		t.Run(curve.Name(), func(t *testing.T) {
			...
			committer, err := pedersenvectorcommitments.NewVectorCommitter(sessionId, curve, prng)
			require.NoError(t, err)
			require.NotNil(t, committer)

			verifier, err := pedersenvectorcommitments.NewVectorVerifier(sessionId, curve)
			require.NoError(t, err)
			require.NotNil(t, verifier)

			vector := make([]pedersenvectorcommitments.Message, i+1)
			for j := 0; j <= i; j++ {
...
func TestHappyPathCombine(t *testing.T) {
	...
	for i, curve := range supportedCurves {
		t.Run(curve.Name(), func(t *testing.T) {
			...
			vectorA := make([]pedersenvectorcommitments.Message, i+1)
			vectorB := make([]pedersenvectorcommitments.Message, i+1)
			vectorAplusB := make([]pedersenvectorcommitments.Message, i+1)
			for j := 0; j <= i; j++ {
