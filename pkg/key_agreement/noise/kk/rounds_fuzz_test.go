...
func Fuzz_KK(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, hashIndex uint, curveIndex uint, aliceSecret uint64, bobSecret uint64, sid []byte, message []byte, handshakeMessage1 []byte, handshakeMessage2 []byte, randomSeed int64) {
		...
		aliceSession, err := kk.NewInitiator(suite, prng, sid, aliceIdentity, bobIdentity.PublicKey, handshakeMessage1, handshakeMessage2)
		if err != nil {
		...
		bobSession, err := kk.NewResponder(suite, prng, sid, bobIdentity, aliceIdentity.PublicKey, handshakeMessage1, handshakeMessage2)
		if err != nil {
			if !errs.IsKnownError(err) {
				require.NoError(t, err)
			} else {
