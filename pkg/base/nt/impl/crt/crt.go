...
func Recombine[M internal.ModulusMutablePtr[N, MT], N internal.NatMutablePtr[N, NT], MT, NT any](
	mp, mq, p, q N,
) (N, ct.Bool) {
	var pModulusT MT
	ok1 := M(&pModulusT).SetNat(p)
	params, ok2 := Precompute(M(&pModulusT), q)
	return params.Recombine(mp, mq), ok1 & ok2
}

func Precompute[M internal.ModulusMutablePtr[N, MT], N internal.NatMutablePtr[N, NT], MT, NT any](p M, q N) (*Params[M, N, MT, NT], ct.Bool) {
	ok := p.Nat().Coprime(q)
	// qModP := q mod p
	var qModPT NT
	p.Mod(N(&qModPT), q)
...
type Params[M internal.ModulusMutablePtr[N, MT], N internal.NatMutablePtr[N, NT], MT, NT any] struct {
	P    MT
	Q    NT
	QInv NT
}
...
func (prm *Params[M, N, MT, NT]) Recombine(mp, mq N) N {
	// h = (mp - mq) mod p
	var h NT
...
func (prm *Params[M, N, MT, NT]) Extended() *ParamsExtended[M, N, MT, NT] {
	var pNat NT
	N(&pNat).Set(M(&prm.P).Nat())
	var qModulus MT
	M(&qModulus).SetNat(N(&prm.Q))
	return &ParamsExtended[M, N, MT, NT]{
		Params:   *prm,
		PNat:     pNat,
		QModulus: qModulus,
	}
...
func PrecomputeExtended[M internal.ModulusMutablePtr[N, MT], N internal.NatMutablePtr[N, NT], MT, NT any](p M, q N) (*ParamsExtended[M, N, MT, NT], ct.Bool) {
...
type ParamsExtended[M internal.ModulusMutablePtr[N, MT], N internal.NatMutablePtr[N, NT], MT, NT any] struct {
	Params[M, N, MT, NT]
	PNat     NT
	QModulus MT
}

func (prmx *ParamsExtended[M, N, MT, NT]) Decompose(m M) (mp, mq N) {
...
func (prmx *ParamsExtended[M, N, MT, NT]) DecomposeParallel(m M) (mp, mq N) {
	var mpt, mqt NT
...
func (prmx *ParamsExtended[M, N, MT, NT]) DecomposeSerial(m M) (mp, mq N) {
	var mpt, mqt NT
	// Compute m.Nat() mod p
	M(&prmx.P).Mod(N(&mpt), m.Nat())
	// Compute m.Nat() mod q
