...
func TestRSAGroup_Operations(t *testing.T) {
	...
	t.Run("Random", func(t *testing.T) {
		t.Parallel()
		u1, err := group.Random(pcg.NewRandomised())
		require.NoError(t, err)
		require.NotNil(t, u1)

		u2, err := group.Random(pcg.NewRandomised())
		require.NoError(t, err)
		require.NotNil(t, u2)
	...
	t.Run("Multiplication", func(t *testing.T) {
		t.Parallel()
		u1, _ := group.Random(pcg.NewRandomised())
		u2, _ := group.Random(pcg.NewRandomised())
	...
	t.Run("Inversion", func(t *testing.T) {
		t.Parallel()
		u, _ := group.Random(pcg.NewRandomised())
...
func TestRSAGroup_ForgetLearnOrder(t *testing.T) {
	...
	u, err := knownGroup.Random(pcg.NewRandomised())
	require.NoError(t, err)
...
func TestPaillierGroup_EmbedRSA(t *testing.T) {
	...
	// Create RSA element
	rsaElement, err := rsaGroup.Random(pcg.NewRandomised())
	require.NoError(t, err)
...
func TestRSAGroup_RandomSampling_Performance(t *testing.T) {
	...
	for range 100 {
		u, err := group.Random(pcg.NewRandomised())
		require.NoError(t, err)
		require.NotNil(t, u)
	}
}
...
func TestRSAGroupElement_CBOR_KnownOrder(t *testing.T) {
	...
	original, err := group.Random(pcg.NewRandomised())
	require.NoError(t, err)
...
func TestRSAElement_MultiplicationWithDifferentGroups_ShouldPanic(t *testing.T) {
	...
	p1, q1, err := nt.GeneratePrimePair(num.NPlus(), rsaGroupLen/2, pcg.NewRandomised())
	require.NoError(t, err)
	p2, q2, err := nt.GeneratePrimePair(num.NPlus(), rsaGroupLen/2, pcg.NewRandomised())
	require.NoError(t, err)
	...
	u1, err := group1.Random(pcg.NewRandomised())
	require.NoError(t, err)
	u2, err := group2.Random(pcg.NewRandomised())
	require.NoError(t, err)
...
func TestPaillierGroup_InvalidNSquared_ShouldFail(t *testing.T) {
	...
	p, q, _ := nt.GeneratePrimePair(num.NPlus(), rsaGroupLen/2, pcg.NewRandomised())
	n := p.Mul(q)
...
func TestRSAGroup_CBOR_NonCoprimeElement_ShouldFail(t *testing.T) {
	...
	validElem, err := group.Random(pcg.NewRandomised())
	require.NoError(t, err)
...
func BenchmarkPaillierGroup_RandomSampling_KnownOrder(b *testing.B) {
	...
	b.ResetTimer()
	for range b.N {
		_, err := group.Random(pcg.NewRandomised())
		if err != nil {
			b.Fatal(err)
		}
	}
