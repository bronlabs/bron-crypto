...
func PaillierUnitGenerator(t *testing.T) (*rapid.Generator[*znstar.PaillierGroupElementUnknownOrder], *znstar.PaillierGroupUnknownOrder) {
	t.Helper()
	group := errs.Must1(znstar.SamplePaillierGroup(paillierGroupNLen, false, false, pcg.NewRandomised()))
...
func TestPaillierGroup_Representative_Property(t *testing.T) {
	...
	group := errs.Must1(znstar.SamplePaillierGroup(paillierGroupNLen, false, false, prng))
...
func TestPaillierGroup_EmbedRSA_Property_Property(t *testing.T) {
	t.Parallel()
	prng := pcg.NewRandomised()
	paillierGroup := errs.Must1(znstar.SamplePaillierGroup(paillierGroupNLen, false, false, prng))
