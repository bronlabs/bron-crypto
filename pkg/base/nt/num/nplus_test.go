...
func TestPositiveNaturalNumbers_Properties(t *testing.T) {
	...
	require.False(t, np.Order().IsFinite())
...
func TestNatPlus_TryRsh(t *testing.T) {
	t.Parallel()
	...
	t.Run("result positive", func(t *testing.T) {
		...
		for _, tc := range cases {
			n, _ := num.NPlus().FromUint64(tc.value)
			result, err := n.TryRsh(tc.shift)
			require.NoError(t, err)
			require.Equal(t, tc.expected, result.Big().Uint64())
		}
	})
...
func TestNatPlus_TrueLen_AnnouncedLen(t *testing.T) {
	t.Parallel()
	np, _ := num.NPlus().FromUint64(255)
	require.Positive(t, np.TrueLen())
	require.True(t, np.AnnouncedLen() >= np.TrueLen())
}

func TestNatPlus_TrueLen_IsBits(t *testing.T) {
	t.Parallel()
	cases := []struct {
		value   uint64
		wantBit int
	...
	for _, tc := range cases {
		np, err := num.NPlus().FromUint64(tc.value)
		require.NoError(t, err)
		require.Equal(t, tc.wantBit, np.TrueLen(),
			"TrueLen must be in bits: value=%d expected %d bits, got %d",
			tc.value, tc.wantBit, np.TrueLen())
	}
}

func TestNatPlus_AnnouncedLen_IsBits(t *testing.T) {
	t.Parallel()
	np, err := num.NPlus().FromUint64(1)
	require.NoError(t, err)
	require.Equal(t, 64, np.AnnouncedLen())
}
