...
func (u *Uint) EuclideanDiv(other *Uint) (quot, rem *Uint, err error) {
	errs.Must1(u.isValid(other))
	if !u.Group().IsDomain() {
		return nil, nil, ErrUndefined.WithMessage("not a euclidean domain")
	}

	var q, r numct.Nat
	if ok := q.EuclideanDiv(&r, u.v, other.v); ok == ct.False {
		return nil, nil, ErrUndefined.WithMessage("division failed")
	}
	u.m.Mod(&q, &q)
	u.m.Mod(&r, &r)
...
func (u *Uint) TryInv() (*Uint, error) {
	if !u.IsUnit() {
		return nil, ErrUndefined.WithMessage("not a unit")
	}
	v := new(numct.Nat)
	u.m.ModInv(v, u.v)
...
func (u *Uint) TryDiv(other *Uint) (*Uint, error) {
	...
	if ok := u.m.ModDiv(v, u.v, other.v); ok == ct.False {
		return nil, ErrUndefined.WithMessage("division failed")
	}
	return &Uint{v: v, m: u.m}, nil
}
...
func (u *Uint) Sqrt() (*Uint, error) {
	...
	if ok := u.m.ModSqrt(v, u.v); ok == ct.False {
		return nil, ErrUndefined.WithMessage("square root failed")
	}
	return &Uint{v: v, m: u.m}, nil
}
