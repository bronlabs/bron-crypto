...
func NewPaillierGroup(p, q *num.NatPlus) (PaillierGroupKnownOrder, error) {
	...
	return &paillierGroupKnownOrder{
		DenseUZMod: DenseUZMod[*modular.OddPrimeSquareFactors]{
			zMod:  zMod,
			arith: exp,
		},
		n: n,
...
func (pg *paillierGroup) FromUint(input *num.Uint) (Unit, error) {
	u, err := pg.DenseUZMod.FromUint(input)
	if err != nil {
		return nil, err
	}
	u.(*unit).g = pg
...
func (pg *paillierGroup) FromNatCT(input *numct.Nat) (Unit, error) {
	u, err := pg.DenseUZMod.FromNatCT(input)
	if err != nil {
		return nil, err
	}
	u.(*unit).g = pg
...
func (pg *paillierGroup) Random(prng io.Reader) (Unit, error) {
	u, err := pg.DenseUZMod.Random(prng)
	if err != nil {
		return nil, err
	}
	u.(*unit).g = pg
...
func (pg *paillierGroup) FromBytes(input []byte) (Unit, error) {
	u, err := pg.DenseUZMod.FromBytes(input)
	if err != nil {
		return nil, err
	}
	u.(*unit).g = pg
...
func (pg *paillierGroup) FromCardinal(input cardinal.Cardinal) (Unit, error) {
	u, err := pg.DenseUZMod.FromCardinal(input)
	if err != nil {
		return nil, err
	}
	u.(*unit).g = pg
...
func (pg *paillierGroup) FromUint64(value uint64) (Unit, error) {
	u, err := pg.DenseUZMod.FromUint64(value)
	if err != nil {
		return nil, err
	}
	u.(*unit).g = pg
...
func (pg *paillierGroup) EmbedRSA(rsaUnit Unit) (Unit, error) {
	...
	v, err := num.NewUintGivenModulus(rsaUnit.Value().Value(), pg.ModulusCT())
	if err != nil {
...
func (pg *paillierGroup) LiftToNthResidues(rsaUnit Unit) (Unit, error) {
	...
	lifted, err := pg.FromNatCT(rsaUnit.Value().Value())
	if err != nil {
...
type paillierGroupKnownOrder struct {
	DenseUZMod[*modular.OddPrimeSquareFactors]
...
func (pg *paillierGroupKnownOrder) LiftToNthResidues(rsaUnit Unit) (Unit, error) {
	...
	var out numct.Nat
	pg.arith.ExpToN(&out, lifted.Value().Value())
	v, err := num.NewUintGivenModulus(&out, pg.ModulusCT())
	if err != nil {
