...
// New returns the U128 value (lo, hi).
...
func NewFromBytesBE(src []byte) U128 {
	var buffer [16]byte
	if len(src) > 16 {
		copy(buffer[:], src[16-len(src):])
	} else {
		copy(buffer[16-len(src):], src)
	}

	var dst U128
	dst.Lo = binary.BigEndian.Uint64(buffer[8:])
	dst.Hi = binary.BigEndian.Uint64(buffer[:8])
	return dst
}
...
func NewFromBytesLE(src []byte) U128 {
	...
	var dst U128
	dst.Lo = binary.LittleEndian.Uint64(buffer[:8])
	dst.Hi = binary.LittleEndian.Uint64(buffer[8:])
	return dst
}
...
func (U128) SetNat(v *saferith.Nat) U128 {
	return NewFromNat(v)
}
...
func (U128) SetBytes(bytes []byte) (U128, error) {
	if len(bytes) > 16 {
...
func (U128) SetBytesWide(bytes []byte) (U128, error) {
	return NewFromBytesBE(bytes), nil
}
