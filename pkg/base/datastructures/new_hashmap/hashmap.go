...
type HashMap[K Hashable[K], V any] struct {
	inner map[uint64][]*entry[K, V]
}

func NewHashMap[K Hashable[K], V any]() *HashMap[K, V] {
	return &HashMap[K, V]{
		inner: make(map[uint64][]*entry[K, V]),
	}
}

func (m *HashMap[K, V]) Get(key K) (value V, exists bool) {
	var nilValue V
...
func (m *HashMap[K, V]) Put(key K, newValue V) (replaced bool, oldValue V) {
	var nilValue V
...
func (m *HashMap[K, V]) Remove(key K) (removed bool, removedValue V) {
	var nilValue V
