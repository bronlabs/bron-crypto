...
func (*IntegerAdapter[T]) Wrap(x ObjectUnderlyer) T {
	var out T
	mod := ObjectUnderlyer(reflect.TypeOf(out).Size())
	return T(x % mod)
}

func (*IntegerAdapter[T]) Unwrap(x T) ObjectUnderlyer {
	return ObjectUnderlyer(x)
}

func (*IntegerAdapter[T]) ZeroValue() T {
	return T(0)
}
...
func (*StringAdapter[T]) Wrap(x ObjectUnderlyer) T {
	return T(strconv.Itoa(int(x)))
}

func (*StringAdapter[T]) Unwrap(x T) ObjectUnderlyer {
...
func (*StringAdapter[T]) ZeroValue() T {
	return T("")
}
...
func (*ByteAdapter[T]) Wrap(x ObjectUnderlyer) T {
	return T(x % 8)
}

func (*ByteAdapter[T]) Unwrap(x T) ObjectUnderlyer {
	return ObjectUnderlyer(x)
}

func (*ByteAdapter[T]) ZeroValue() T {
	return T(byte(0))
}
...
func (*SliceAdapter[S, O]) ZeroValue() S {
	return make(S, 0)
}
...
func (*NativeMapAdapter[M, K, V]) ZeroValue() M {
	return map[K]V{}
}
...
func (ListElementAdapter[T]) ZeroValue() T {
	return *new(T)
}
