...
func RandomBigInt(prng io.Reader, min, max *big.Int, nonZero bool) (*big.Int, error) {
	if err := validateRandomBigInt(prng, min, max, nonZero); err != nil {
		return nil, errs.WrapArgument(err, "invalid arguments")
	}
	rangeSize := new(big.Int).Add(new(big.Int).Sub(max, min), big.NewInt(1))
	for {
...
func validateRandomBigInt(prng io.Reader, min, max *big.Int, nonZero bool) error {
	if prng == nil {
...
func RandomInteger[T constraints.Integer](prng io.Reader, min, max T, nonZero bool) (T, error) {
	if err := validateRandomInt(prng, min, max, nonZero); err != nil {
		return 0, errs.WrapArgument(err, "invalid argument")
	}
	signedT := IsSigned(min)
	var maxBig *big.Int
	var minBig *big.Int
	if signedT {
		...
		minBig = new(big.Int).SetUint64(uint64(min))
	}
...
func validateRandomInt[T constraints.Integer](prng io.Reader, min, max T, nonZero bool) error {
	if prng == nil {
		return errs.NewIsNil("prng")
	}
	if max < min {
...
func RandomSlice[S ~[]T, T comparable](prng io.Reader, sliceLength int, distinct bool, notAllZero bool, sampler func() (T, error)) (S, error) {
