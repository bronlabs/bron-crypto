...
func UniqueFunc[T any](xs []T, isEqual func(a, b T) bool) []T {
	out := []T{}
OUTER:
	for i, xi := range xs {
		for j, xj := range xs {
...
func FilterOut[T comparable](xs []T, shouldDrop ...T) []T {
	h := NativeHashSet(shouldDrop)
	out := []T{}
	for _, x := range xs {
		if _, exists := h[x]; !exists {
			out = append(out, x)
		}
	}
	return out
...
func NativeHashSet[T comparable](xs []T) map[T]bool {
	h := make(map[T]bool)
	for _, x := range xs {
		h[x] = true
	}
