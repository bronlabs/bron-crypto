...
func (s *DirectPowerSemiGroup[S, E, W, WT]) FromBytes(input []byte) (W, error) {
	...
	if len(input)%componentSize != 0 {
		return nil, ErrInvalidArgument.WithMessage("input length is not a multiple of element size")
	}
	...
	for i := 0; i < len(input); i += componentSize {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to decode component %d", i/componentSize)
		}
...
func (d *DirectPowerSemiGroupElement[E, W, WT]) HashCode() base.HashCode {
	...
	for _, x := range d.components {
		...
		if _, err := h.Write(buf[:]); err != nil {
			panic(errs.Wrap(err).WithMessage("could not write to hash function"))
		}
	}
...
func (d *DirectPowerRingElement[E, W, WT]) TrySub(x W) (W, error) {
	...
	for i, c := range d.components {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to subtract component %d", i)
		}
	}
	var out WT
	if err := W(&out).set(d.arity, values...); err != nil {
...
func (d *DirectPowerRingElement[E, W, WT]) TryDiv(x W) (W, error) {
	...
	for i, c := range d.components {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to divide component %d", i)
		}
	}
	var out WT
	if err := W(&out).set(d.arity, values...); err != nil {
...
func (d *DirectPowerRingElement[E, W, WT]) TryInv() (W, error) {
	...
	for i, c := range d.components {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to invert component %d", i)
		}
	}
	var out WT
	if err := W(&out).set(d.arity, values...); err != nil {
...
func (d *DirectPowerRingElement[E, W, WT]) TryNeg() (W, error) {
	...
	for i, c := range d.components {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to negate component %d", i)
		}
	}
	var out WT
	if err := W(&out).set(d.arity, values...); err != nil {
...
func EvalDirectProductOfPolynomialLikes[
	...
	P algebra.UnivariatePolynomialLike[P, S, C, SS, CS],
	C algebra.GroupElement[C],
	S algebra.RingElement[S],
	SS algebra.Structure[S], CS algebra.Structure[C],
](product Ps, at S) ([]C, error) {
	if utils.IsNil(product) {
		return nil, ErrInvalidArgument.WithMessage("product cannot be nil")
	}
	if utils.IsNil(at) {
		return nil, ErrInvalidArgument.WithMessage("evaluation point cannot be nil")
	}
	components := product.Components()
	arity := product.Arity()
	if arity.IsZero() || !arity.IsFinite() {
		return nil, ErrInvalidArgument.WithMessage("product must have finite and positive arity")
	}
...
func (s *DirectPowerOfFiniteStructures[S, E, W, WT]) Random(prng io.Reader) (W, error) {
	...
	for i := range s.dim {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to generate random component %d", i)
		}
	}
	var out WT
...
func (s *DirectPowerOfFiniteStructures[S, E, W, WT]) Hash(input []byte) (W, error) {
	...
	for i := range s.dim {
		...
		if err != nil {
			return nil, errs.Wrap(err).WithMessage("failed to hash component %d", i)
		}
	}
	var out WT
	if err := W(&out).set(s.dim, values...); err != nil {
