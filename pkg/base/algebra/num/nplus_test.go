...
func TestNatPlus_BytesConversion(t *testing.T) {
	...
	for _, test := range bytesTests {
		...
		require.True(t, bytes.Equal(test.expected, result))
...
func TestNatPlus_Random(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		require.True(t, result.Compare(low.Lift().Abs()) >= 0)
		require.True(t, result.Compare(high.Lift().Abs()) < 0)
	}
}
...
func TestNatPlus_RandomWithNilLow(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		require.True(t, result.Compare(num.NPlus().One().Lift().Abs()) >= 0)
		require.True(t, result.Compare(high.Lift().Abs()) < 0)
	}
}
