...
func TestCardinalConstants(t *testing.T) {
	t.Run("Zero", func(t *testing.T) {
		assert.True(t, cardinal.Zero.IsFinite())
		assert.False(t, cardinal.Zero.IsUnknown())
		assert.True(t, cardinal.Zero.IsZero())
		assert.Equal(t, uint64(0), cardinal.Zero.Uint64())
		// String format includes hex representation
		assert.Contains(t, cardinal.Zero.String(), "Cardinal(")
	})

	t.Run("Infinite", func(t *testing.T) {
		assert.False(t, cardinal.Infinite.IsFinite())
		assert.False(t, cardinal.Infinite.IsUnknown())
		// IsZero uses Equal which has issues with nil values
		// assert.False(t, impl.Infinite.IsZero())
		assert.Equal(t, uint64(0), cardinal.Infinite.Uint64())
		assert.Equal(t, "Infinite", cardinal.Infinite.String())
	})

	t.Run("Unknown", func(t *testing.T) {
		assert.False(t, cardinal.Unknown.IsFinite())
		assert.True(t, cardinal.Unknown.IsUnknown())
		// IsZero uses Equal which has issues with nil values
		// assert.False(t, impl.Unknown.IsZero())
		assert.Equal(t, uint64(0), cardinal.Unknown.Uint64())
		assert.Equal(t, "Unknown", cardinal.Unknown.String())
	})
}
...
func TestCardinalImplementationNotes(t *testing.T) {
	t.Run("Zero value behavior", func(t *testing.T) {
		...
		assert.True(t, c1 == c2)
		assert.True(t, c1 == cardinal.Zero)
	})
}
