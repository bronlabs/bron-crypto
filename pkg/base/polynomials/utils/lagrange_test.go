...
func TestLi(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			polynomial, err := polynomialsUtils.Li(polynomialsSet, tt.i, xsScalar)
			require.NoError(t, err)

			actual := polynomial.Eval(xScalar)
...
func TestAllBasisPolynomials(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			polynomialsSet := polynomials.GetScalarUnivariatePolynomialsSet(curve.ScalarField())
			lagrangePolynomialsBasis, err := polynomialsUtils.LagrangeBasis(polynomialsSet, xsScalar)
			require.NoError(t, err)

			actual := make(map[int]curves.Scalar)
			for k, p := range lagrangePolynomialsBasis {
				actual[k] = p.Eval(xScalar)
			}
			require.Exactly(t, expectedScalar, actual)
		})
	}
