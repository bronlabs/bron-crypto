...
func TestNewPoly(t *testing.T) {
	...
	poly, err := polynomials.NewRandomPolynomial(secret, 4, crand.Reader)
	require.NoError(t, err)
...
func TestLi(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			xScalar := curve.ScalarField().FromUint64(uint64(sign * tt.x))
			if sign == -1 {
				xScalar = xScalar.Neg()
			}
			xsScalar := make([]*k256.Scalar, len(tt.xs))
			for i := 0; i < len(tt.xs); i++ {
				sign := 1
				if tt.xs[i] < 0 {
					sign = -1
				}
				xsScalar[i] = curve.ScalarField().FromUint64(uint64(sign * tt.xs[i]))
				if sign == -1 {
			...
			expectedScalar := curve.ScalarField().FromUint64(uint64(sign2 * tt.expected))
			if sign2 == -1 {
				expectedScalar = expectedScalar.Neg()
			}
			actual, err := lagrange.L_i(tt.i, xsScalar, xScalar)
			require.NoError(t, err)
...
func TestAllBasisPolynomials(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			xScalar := curve.ScalarField().FromUint64(uint64(signX * tt.x))
			if signX == -1 {
				xScalar = xScalar.Neg()
			}
			xsScalar := make([]*k256.Scalar, len(tt.xs))
			for i := 0; i < len(tt.xs); i++ {
				...
				xsScalar[i] = curve.ScalarField().FromUint64(uint64(signXs * tt.xs[i]))
				if signXs == -1 {
					xsScalar[i] = xsScalar[i].Neg()
				}
			}
			expectedScalar := make(map[int]*k256.Scalar, len(tt.expected))
			for k, v := range tt.expected {
				signV := 1
				if v < 0 {
					signV = -1
				}
				expectedScalar[k] = curve.ScalarField().FromUint64(uint64(signV * v))
				if signV == -1 {
					expectedScalar[k] = expectedScalar[k].Neg()
				}
			}
			actual, err := lagrange.Basis(xsScalar, xScalar)
			require.NoError(t, err)
			require.Exactly(t, expectedScalar, actual)
		})
	}
