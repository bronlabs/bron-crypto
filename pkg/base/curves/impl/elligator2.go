...
func (el2 *Elligator2Params) MapToCurveElligator2Curve25519(u curves.BaseFieldElement) (xn, xd, y, z curves.BaseFieldElement) {
	...
	tv2 = y11.Square()                      /* 19 */
	tv2 = tv2.Mul(gxd)                      /* 20 */
	e1 := utils.BoolTo[int](tv2.Equal(gx1)) /* 21 */
	...
	tv2 = y21.Square()                      /* 28 */
	tv2 = tv2.Mul(gxd)                      /* 29 */
	e2 := utils.BoolTo[int](tv2.Equal(gx2)) /* 30 */
	y2 := F.Select(e2, y22, y21)            /* 31 */
	tv2 = y1.Square()                       /* 32 */
	tv2 = tv2.Mul(gxd)                      /* 33 */
	e3 := utils.BoolTo[int](tv2.Equal(gx1)) /* 34 */
	xn = F.Select(e3, x2n, x1n)             /* 35 */
	y = F.Select(e3, y2, y1)                /* 36 */
	e4 := utils.BoolTo[int](y.IsOdd())      /* 37 */
	y = F.Select((e3-e4)&1, y, y.Neg())     /* 38 */
