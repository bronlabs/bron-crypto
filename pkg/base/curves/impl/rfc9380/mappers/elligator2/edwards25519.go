...
func mapToCurveElligator2Edwards25519[FP fieldsImpl.PrimeFieldElementPtr[FP, F], F any](xnOut, xdOut, ynOut, ydOut, u *F) {
	...
	e := FP(&tv1).IsZero()
	//  9.  xn = CMOV(xn, 0, e)
	FP(&xn).CondAssign(e, &xn, &zero)
	// 10. xd = CMOV(xd, 1, e)
	FP(&xd).CondAssign(e, &xd, &one)
	// 11. yn = CMOV(yn, 1, e)
	FP(&yn).CondAssign(e, &yn, &one)
	// 12. yd = CMOV(yd, 1, e)
	FP(&yd).CondAssign(e, &yd, &one)
