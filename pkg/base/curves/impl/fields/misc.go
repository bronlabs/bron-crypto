...
func IsNegative[FP PrimeFieldElementPtrConstraint[FP, F], F any](v *F) (neg uint64) {
	var vNeg F
	FP(&vNeg).Neg(v)
	return greaterLimbs(FP(v).Limbs(), FP(&vNeg).Limbs())
}

func IsOdd[FP PrimeFieldElementPtrConstraint[FP, F], F any](v *F) (odd uint64) {
	return uint64(FP(v).Bytes()[0] & 0b1)
}

func Degree[FP FiniteFieldElementPtrConstraint[FP, F], F any]() uint64 {
