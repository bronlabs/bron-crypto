...
func init() {
	...
	plutoGenerator.X.Neg(new(Fp).SetUint64(2))
	plutoGenerator.Y.SetUint64(7)
	plutoGenerator.Z.SetOne()
}
...
func (p *PlutoPoint) Identity() *PlutoPoint {
	p.X.SetZero()
	p.Y.SetOne()
	p.Z.SetZero()
	return p
...
func (p *PlutoPoint) Generator() *PlutoPoint {
	p.Set(&plutoGenerator)
	return p
}

// IsIdentity returns true if this point is at infinity.
func (p *PlutoPoint) IsIdentity() uint64 {
	return p.Z.IsZero()
}

// IsOnCurve determines if this point represents a valid curve point.
func (p *PlutoPoint) IsOnCurve() uint64 {
	// Y^2 Z = X^3 + b Z^3
	var lhs, rhs, t Fp
	lhs.Square(&p.Y)
	lhs.Mul(&lhs, &p.Z)
...
func (*PlutoPoint) InCorrectSubgroup() uint64 {
	return 1
}

// Add adds this point to another point.
func (p *PlutoPoint) Add(arg1, arg2 *PlutoPoint) *PlutoPoint {
...
func (p *PlutoPoint) Sub(arg1, arg2 *PlutoPoint) *PlutoPoint {
	var t PlutoPoint
	t.Neg(arg2)
	return p.Add(arg1, &t)
}
...
// Double this point.
func (p *PlutoPoint) Double(a *PlutoPoint) (a2 *PlutoPoint) {
...
func (p *PlutoPoint) Mul(a *PlutoPoint, s *Fq) *PlutoPoint {
	bytes := s.Bytes()
	return p.multiply(a, &bytes)
}

func (p *PlutoPoint) multiply(a *PlutoPoint, bytes *[FieldBytes]byte) *PlutoPoint {
	var t PlutoPoint
	precomputed := [16]*PlutoPoint{}
	precomputed[0] = new(PlutoPoint).Identity()
	precomputed[1] = new(PlutoPoint).Set(a)
	for i := 2; i < 16; i += 2 {
		precomputed[i] = new(PlutoPoint).Double(precomputed[i>>1])
		precomputed[i+1] = new(PlutoPoint).Add(precomputed[i], a)
	}
	t.Identity()
...
func (p *PlutoPoint) MulByU(a *PlutoPoint) *PlutoPoint {
	// Skip first bit since its always zero
	var s, t, r PlutoPoint
	r.Identity()
	t.Set(a)
...
func (p *PlutoPoint) ClearCofactor(a *PlutoPoint) *PlutoPoint {
	p.Set(a)
	return p
}

// Neg negates this point.
func (p *PlutoPoint) Neg(a *PlutoPoint) *PlutoPoint {
	p.Set(a)
...
// Set copies a into p.
func (p *PlutoPoint) Set(a *PlutoPoint) *PlutoPoint {
	p.X.Set(&a.X)
	p.Y.Set(&a.Y)
	p.Z.Set(&a.Z)
	return p
...
func (p *PlutoPoint) Nat() (x, y *saferith.Nat) {
	var t PlutoPoint
	t.ToAffine(p)
...
func (p *PlutoPoint) SetNat(x, y *saferith.Nat) (*PlutoPoint, error) {
	var xx, yy Fp
	var pp PlutoPoint
	pp.X = *(xx.SetNat(x))
	pp.Y = *(yy.SetNat(y))
...
func (p *PlutoPoint) ToAffine(a *PlutoPoint) *PlutoPoint {
...
func (p *PlutoPoint) Equal(rhs *PlutoPoint) uint64 {
	var x1, x2, y1, y2 Fp
	var e1, e2 uint64
...
func (p *PlutoPoint) CMove(arg1, arg2 *PlutoPoint, choice uint64) *PlutoPoint {
