...
var (
	...
	erisBaseFieldInstance BaseField
)
...
func NewErisBaseField() *BaseField {
	erisBaseFieldInitOnce.Do(erisBaseFieldInit)
	return &erisBaseFieldInstance
}
...
func (*BaseField) Hash(x []byte) (curves.BaseFieldElement, error) {
	els, err := NewErisCurve().HashToFieldElements(1, x, nil)
	if err != nil {
		return nil, errs.WrapHashing(err, "could not hash to field element in eris base field")
	}
	return els[0], nil
}

func (*BaseField) Select(choice bool, x0, x1 curves.BaseFieldElement) curves.BaseFieldElement {
	x0f, ok0 := x0.(*ErisBaseFieldElement)
	x1f, ok1 := x1.(*ErisBaseFieldElement)
	if !ok0 || !ok1 {
		panic("Not a field element")
	}
	el := new(ErisBaseFieldElement)
...
func (*BaseField) QuadraticResidue(p algebra.RingElement[curves.BaseField, curves.BaseFieldElement]) (curves.BaseFieldElement, error) {
	pp, ok := p.Unwrap().(*ErisBaseFieldElement)
	if !ok {
