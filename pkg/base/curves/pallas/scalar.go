...
func (*Scalar) SetBytes(input []byte) (curves.Scalar, error) {
	...
	value, err := new(fq.Fq).SetBytes((*[constants.FieldBytes]byte)(input))
	if err != nil {
...
func (*Scalar) SetBytesWide(input []byte) (curves.Scalar, error) {
	if len(input) > constants.WideFieldBytes {
		return nil, errs.NewInvalidLength("invalid length %d > %d bytes", len(input), constants.WideFieldBytes)
	}
	input = bitstring.ReverseAndPadBytes(input, constants.WideFieldBytes-len(input))
	return &Scalar{
		Value: new(fq.Fq).SetBytesWide((*[constants.WideFieldBytes]byte)(input)),
	}, nil
}
