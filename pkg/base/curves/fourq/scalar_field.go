...
var (
	fourQScalarFieldInitOnce sync.Once
	fourQScalarFieldInstance ScalarField
)
...
func fourQScalarFieldInit() {
	fourQScalarFieldInstance = ScalarField{}
}

func NewScalarField() *ScalarField {
	fourQScalarFieldInitOnce.Do(fourQScalarFieldInit)
	return &fourQScalarFieldInstance
}
...
func (*ScalarField) Select(choice uint64, x0, x1 curves.Scalar) curves.Scalar {
	x0s, ok0 := x0.(*Scalar)
	if !ok0 || x0s.V == nil {
		panic("x0 is not a non-empty FourQ scalar")
	}
	x1s, ok1 := x1.(*Scalar)
	if !ok1 || x1s.V == nil {
		panic("x1 is not a non-empty FourQ scalar")
	}

	return &Scalar{V: new(impl.Fq).CMove(x0s.V, x1s.V, choice)}
}
