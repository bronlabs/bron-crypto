...
func TestFpSetOne(t *testing.T) {
	t.Parallel()
	fpOne := fp.New().SetOne()
	var fpRandom *limb4.FieldValue
	for fpRandom == nil || fpRandom.IsZero() == 1 {
		var randomBytes [64]byte
		_, err := io.ReadFull(crand.Reader, randomBytes[:])
		require.NoError(t, err)
		fpRandom = fp.New().SetBytesWide(&randomBytes)
	}
	require.NotNil(t, fpRandom)
	require.False(t, fpRandom.IsZero() == 1)
	fpOneTimesRandom := fp.New().Mul(fpOne, fpRandom)
	require.NotNil(t, fpOneTimesRandom)
	require.True(t, fpRandom.Equal(fpOneTimesRandom) == 1)
...
func TestFpNeg(t *testing.T) {
	t.Parallel()
	g := fp.New().SetUint64(7)
	a := fp.New().SetOne()
	a.Neg(a)
	e := fp.New().SetRaw(&[limb4.FieldLimbs]uint64{0xfffffffffffffffe, 0x00000001ffffffff, 0x0000000000000000, 0xfffffffe00000002})
	require.Equal(t, e, a)
	a.Neg(g)
	e = fp.New().SetRaw(&[limb4.FieldLimbs]uint64{0xfffffffffffffff8, 0x00000007ffffffff, 0x0000000000000000, 0xfffffff800000008})
	require.Equal(t, e, a)
}
...
func TestFpInvert(t *testing.T) {
	t.Parallel()
	twoInv := fp.New().SetRaw(&[limb4.FieldLimbs]uint64{0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x8000000000000000})
	two := fp.New().SetUint64(2)
	a, inverted := fp.New().Invert(two)
	require.True(t, inverted)
	require.Equal(t, a, twoInv)

	rootOfUnity := fp.New().SetLimbs(&[limb4.FieldLimbs]uint64{0x8619a9e760c01d0c, 0xa883c4fba37998df, 0x45607580b6eabd98, 0xf252b002544b2f99})
	rootOfUnityInv := fp.New().SetRaw(&[limb4.FieldLimbs]uint64{0x2609d8ab477f96d1, 0x6e5f128fb20a0f24, 0xe4d636874d99643b, 0x376080cc1f2a3735})
	a, inverted = fp.New().Invert(rootOfUnity)
	require.True(t, inverted)
...
func TestFpBigInt(t *testing.T) {
	...
	e := fp.New().SetRaw(&[limb4.FieldLimbs]uint64{0x515151513f3f3f3e, 0xc9c9c9cb36363636, 0xb7b7b7b79c9c9c9c, 0xfffffffeaeaeaeaf})
	...
	b = new(saferith.Nat).ModNeg(b, e.Params.Modulus)
	t1.SetNat(b)
	require.Equal(t, e, t1)
}
...
func TestFpSetBytesWide(t *testing.T) {
	t.Parallel()
	e := fp.New().SetRaw(&[limb4.FieldLimbs]uint64{0xccdefd48c77805bc, 0xe935dc2db86364d6, 0xca8ee6e5870a020e, 0x4c94bf4467f3b5bf})
