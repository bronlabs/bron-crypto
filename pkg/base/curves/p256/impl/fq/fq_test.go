...
func TestFqNeg(t *testing.T) {
	...
	a := New().SetOne()
	a.Neg(a)
	e := New().SetRaw(&[impl.FieldLimbs]uint64{0xe7739585f8c64aa2, 0x79cdf55b4e2f3d09, 0xffffffffffffffff, 0xfffffffe00000001})
	require.Equal(t, e, a)
	a.Neg(g)
	e = New().SetRaw(&[impl.FieldLimbs]uint64{0x9dce5617e3192a88, 0xe737d56d38bcf427, 0xfffffffffffffffd, 0xfffffff800000007})
	require.Equal(t, e, a)
}
...
func TestFqInvert(t *testing.T) {
	twoInv := New().SetRaw(&[impl.FieldLimbs]uint64{0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x8000000000000000})
	two := New().SetUint64(2)
	a, inverted := New().Invert(two)
	require.True(t, inverted)
	require.Equal(t, a, twoInv)

	rootOfUnityInv := New().SetRaw(&[impl.FieldLimbs]uint64{0xcfbf53b9618acf96, 0xf17e5c39df7bd05b, 0xc7acb1f83e3ad9ad, 0x4659a42b394ff7df})
...
func TestFqBigInt(t *testing.T) {
	...
	e := New().SetRaw(&[impl.FieldLimbs]uint64{0x21dcaaadf1cb6aa0, 0x568de5f5990a98d7, 0x354f43b0d837fac5, 0x3e02532cb23f481a})
...
func TestFqSetBytesWide(t *testing.T) {
	e := New().SetRaw(&[impl.FieldLimbs]uint64{0xe2b8d4b0e576c8fa, 0x9d2b215f85d3bdf7, 0xf6070a872442640c, 0xcf15d1e49c990b88})
