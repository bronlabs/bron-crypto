...
func (*PairingCurve) Pair(pG1, pG2 curves.PairingPoint) (curves.GtMember, error) {
	p1, ok := pG1.(*PointG1)
	if !ok {
		return nil, errs.NewInvalidType("first point is not in G1")
	}
	p2, ok := pG2.(*PointG2)
	if !ok {
		return nil, errs.NewInvalidType("second point is not in G2")
	}
...
func (*PairingCurve) MultiPair(points ...curves.PairingPoint) (curves.GtMember, error) {
	if len(points)%2 != 0 {
		return nil, errs.NewInvalidLength("#G1 != #G2")
	}
	eng := new(bls12381impl.Engine)
	for i := 0; i < len(points); i += 2 {
		pt1, ok := points[i].(*PointG1)
		if !ok {
			return nil, errs.NewInvalidType("point %d is not in G1", i)
		}
		pt2, ok := points[i+1].(*PointG2)
		if !ok {
			return nil, errs.NewInvalidType("point %d is not G2", i+1)
		}
