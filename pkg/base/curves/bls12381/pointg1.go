...
func (p *PointG1) ApplyAdd(q curves.Point, n *saferith.Nat) curves.Point {
	return p.Add(q.ScalarMul(NewScalarFieldG1().Element().SetNat(n)))
}
...
func (p *PointG1) ApplySub(q curves.Point, n *saferith.Nat) curves.Point {
	return p.Sub(q.ScalarMul(NewScalarFieldG1().Element().SetNat(n)))
}

// === Vector Space Methods.

func (p *PointG1) ScalarMul(rhs curves.Scalar) curves.Point {
	if rhs == nil {
		panic("rhs is nil")
	}
	r, ok := rhs.(*Scalar)
