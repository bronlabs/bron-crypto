...
func Fp6Arithmetic(t *testing.T) {
	t.Helper()
	t.Parallel()
	a := bls12381impl.Fp6{
		A: bls12381impl.Fp2{
	...
	d := new(bls12381impl.Fp6).Square(&a)
	e := new(bls12381impl.Fp6).Mul(&a, &a)
	...
	// (a + b) * c^2
	d.Add(&a, &b)
	d.Mul(d, new(bls12381impl.Fp6).Square(&c))

	e.Mul(&c, &c)
	e.Mul(e, &a)
	tt := new(bls12381impl.Fp6).Mul(&c, &c)
	tt.Mul(tt, &b)
	e.Add(e, tt)
