...
func getEntries() []entry {
	var testResults = []entry{
		{message: []byte("This is a test message"), opening: nil, commitment: nil, err: nil},
		// {opening: NewOpening([]byte("short msg"), nil), commitment: nil, err: nil},
		// {
		// 	opening:    NewOpening([]byte("This input field is intentionally longer than the SHA256 block size to ensure that the entire message is processed"), nil),
		// 	commitment: NewCommitment(nil), err: nil,
		// },
		// {
		// 	opening:    NewOpening([]byte{0xFB, 0x1A, 0x18, 0x47, 0x39, 0x3C, 0x9F, 0x45, 0x5F, 0x29, 0x4C, 0x51, 0x42, 0x30, 0xA6, 0xB9}, nil),
		// 	commitment: NewCommitment(nil), err: nil,
		// },
		// // msg = \epsilon (empty string)
		// {opening: NewOpening([]byte{}, nil), commitment: NewCommitment(nil), err: nil},
		// // msg == nil
		// {opening: NewOpening(nil, nil), commitment: NewCommitment(nil), err: nil},
	}
	for i := range testResults {
		c := NewCommitter([]byte("00000001"))
		entry := &testResults[i]
		entry.commitment, entry.opening, entry.err = c.Commit(crand.Reader, entry.message)
	}
	return testResults
}
