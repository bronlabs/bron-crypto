...
func (ctrDrbg *CtrDRBG) Update(providedData []byte) (err error) {
	...
	tempBlocks := utils.Math.CeilDiv(ctrDrbg.SeedSize(), AesBlockSize)
...
func (ctrDrbg *CtrDRBG) Generate(outputBuffer, additionalInput []byte) (err error) {
	...
	requestedNumberOfBlocks := utils.Math.CeilDiv(requestedNumberOfBytes, AesBlockSize)
...
func (ctrDrbg *CtrDRBG) BlockCipherDF(inputString []byte, noOfBytesToReturn int) (requestedBytes []byte, err error) {
	...
	sBlocks := utils.Math.CeilDiv(int(4+4+l+1), AesBlockSize)
	...
	for i := 0; i < tempBlocks; i++ {
		...
		if i > 0 {
			utils.SliceFill(ivNs[:AesBlockSize], 0)
		}
	...
	requestedBlocks := utils.Math.CeilDiv(noOfBytesToReturn, AesBlockSize)
...
func BCC(aesCipher cipher.Block, data, outputBlock []byte) {
	...
	utils.SliceFill(outputBlock, 0) // chaining_value
	// 2. n = len(data)/outlen.
