% i. Our scenario   [Alberto]
% 1) Note: can take inspiration from FROST spec
% • Async communication
% • Cohort and cohort config
% • Participant roles (signature aggregator, composer, signature requester).
% • Signing modes (cold, warm, hot)
% Disaster recovery - not a concern (missing verifiable encryption).
...
\paragraph{On setting the cohort of participants.} All the protocols assume its set of participants to be formed prior to the execution of the protocol, by using some out-of-band mechanism. We name such a set as \textit{cohort}. Furthermore, in many of these signing protocols, a participant's ID is the point at which the Shamir polynomial is evaluated to output the share for that participant. As a result, each participant will know the participant IDs of all other participants, among other things, before engaging in any of the signing protocols. 
...
\paragraph{On Signing scenarios.} Various copper use cases will require different selections of each role. Below, we point out how various modes of Copper's SASS offering can be realized:
\begin{itemize}
    \item \textbf{Cold Signing:} non-interactive signing, with $\PC=\SR=\{\party{client}\}$ and $\SA=\{\party{copper}\}$.
    \item \textbf{Hot Signing:} This is interactive signing where $\PC=\{\}$ and $\SR=\{\party{client}\}$ and $\SA=\{\party{copper}\}$
